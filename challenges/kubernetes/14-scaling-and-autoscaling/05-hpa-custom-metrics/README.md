...
Custom metrics from Prometheus are powerful but require running Prometheus in-cluster. The next exercise covers scaling from external metrics sources like cloud provider queues and monitoring services: [14.06 - HPA with External Metrics (SQS, CloudWatch)](../06-hpa-external-metrics/).
