...
Now that you understand dynamic provisioning, the next exercise covers the foundational volume types `emptyDir` and `hostPath` and when to use each. Continue to [exercise 03 (Volume Basics: emptyDir and hostPath)](../03-volume-basics-emptydir-hostpath/03-volume-basics-emptydir-hostpath.md).
