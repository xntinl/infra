...
The kubelet manages individual nodes, but detecting and responding to node problems requires a separate mechanism. The next exercise covers the Node Problem Detector and automated remediation: [15.10 - Node Problem Detector and Remediation](../10-node-problem-detector/10-node-problem-detector.md).
