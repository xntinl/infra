...
Edit `main.go` and implement the `generator` function:
...
Now implement the `square` stage. It reads integers from an input channel, squares them, and sends the results to an output channel. This is the middle of the pipeline.
...
Now connect generator and square into a pipeline and consume the final output. This is where the composition happens.
...
func runPipeline() {
    ...
    // Stage 3 (consumer): print results
...
Build a `double` stage that multiplies each value by 2 and chain a four-stage pipeline: generator -> square -> double -> filter(threshold=30) -> print. Verify only values above 30 appear in the output.
