...
```elixir
# bench/chronos_bench.exs — Time-series database ingestion and query latencies
# mix run bench/chronos_bench.exs

defmodule ChronosBench do
  @doc "Measures TSDB compression, ingest throughput, and query latencies."
  def run do
    {:ok, db} = Chronos.Database.start_link()
    
    IO.puts("=== Time-Series Database Benchmark (v3 standard) ===\n")

    # Pre-populate with metric history
    IO.puts("Pre-populating 1M data points across 10k series...")
    prepopulate(db)
    IO.puts("Ready.\n")

    # Benchmark phases
    measure_ingest(db)
    measure_gorilla_compression(db)
    measure_range_queries(db)
    measure_high_cardinality(db)
    measure_aggregations(db)
  end

  defp prepopulate(db) do
    now = System.system_time(:millisecond)
    
    for series_i <- 1..10_000 do
      for minute <- 0..99 do
        ts = now - (100 - minute) * 60_000
        host = "host_#{rem(series_i, 100)}"
        region = "region_#{rem(series_i, 10)}"
        value = 50.0 + :rand.uniform() * 50.0
        
        Chronos.ingest(db, "cpu.usage", %{host: host, region: region}, value, ts)
      end
    end
  end

  defp measure_ingest(db) do
    IO.puts("Phase 1: Ingest throughput (single series, high-cardinality labels)")
    now = System.system_time(:millisecond)
    
    Benchee.run(
      %{
        "ingest — single data point (new series)" => fn ->
          host = "host_#{:rand.uniform(10_000)}"
          region = "region_#{:rand.uniform(100)}"
          value = 50.0 + :rand.uniform() * 50.0
          ts = System.system_time(:millisecond)
          Chronos.ingest(db, "cpu.usage", %{host: host, region: region}, value, ts)
        end,
        "ingest — batch 10 points" => fn ->
          for i <- 1..10 do
            host = "host_#{:rand.uniform(10_000)}"
            region = "region_#{:rand.uniform(100)}"
            value = 50.0 + :rand.uniform() * 50.0
            ts = System.system_time(:millisecond) + i
            Chronos.ingest(db, "cpu.usage", %{host: host, region: region}, value, ts)
          end
        end
      },
  ...
  defp measure_gorilla_compression(db) do
    IO.puts("\nPhase 2: Gorilla compression ratio and decode latency")
    
    # Generate monotonic counter (best case for compression)
    now = System.system_time(:millisecond)
    monotonic_samples = for i <- 1..1000, do: {now + i * 10_000, i * 1.0}
    compressed_mono = Chronos.Gorilla.encode(monotonic_samples)
    mono_ratio = (1000 * 16) / byte_size(compressed_mono)
    
    # Generate random data (worst case for compression)
    random_samples = for i <- 1..1000, do: {now + i * 10_000, :rand.uniform() * 100.0}
    compressed_rand = Chronos.Gorilla.encode(random_samples)
    rand_ratio = (1000 * 16) / byte_size(compressed_rand)
    
    IO.puts("\nGorilla compression ratios:")
    IO.puts("  Monotonic counter:  #{Float.round(mono_ratio, 1)}:1")
    IO.puts("  Random data:        #{Float.round(rand_ratio, 1)}:1")
    IO.puts("  Average case:       #{Float.round((mono_ratio + rand_ratio) / 2, 1)}:1")
    
    # Measure encode/decode latency
    Benchee.run(
      %{
        "encode 1000 samples (Gorilla)" => fn ->
          samples = for i <- 1..1000, do: {now + i * 10_000, :rand.uniform() * 100.0}
          Chronos.Gorilla.encode(samples)
        end,
        "decode 1000 samples (Gorilla)" => fn ->
          samples = for i <- 1..1000, do: {now + i * 10_000, :rand.uniform() * 100.0}
          compressed = Chronos.Gorilla.encode(samples)
          Chronos.Gorilla.decode(compressed)
        end
      },
      parallel: 2,
      time: 5,
  ...
  defp measure_range_queries(db) do
    IO.puts("\nPhase 3: Range query latency (time window, label filter)")
    
    now = System.system_time(:millisecond)
    from = now - 3_600_000  # 1 hour ago
    to = now
    ...
    {_time, latencies} = :timer.tc(fn ->
      for _ <- 1..1000 do
        ...
        host = "host_#{:rand.uniform(100)}"
        _results = Chronos.query(db, "cpu.usage", %{host: host},
          from: from, to: to, aggregate: :avg, step: "5m")
    ...
    Benchee.run(
      %{
        "query — 1h range, avg, 5m step" => fn ->
          host = "host_#{:rand.uniform(100)}"
          Chronos.query(db, "cpu.usage", %{host: host},
            from: from, to: to, aggregate: :avg, step: "5m")
        end,
        "query — 1h range, multiple labels (intersection)" => fn ->
          host = "host_#{:rand.uniform(100)}"
          region = "region_#{:rand.uniform(10)}"
          Chronos.query(db, "cpu.usage", %{host: host, region: region},
            from: from, to: to, aggregate: :max, step: "10m")
        end
      },
      parallel: 4,
      time: 5,
    ...
    compute_percentiles("Range query latency", latencies)
  end

  defp measure_high_cardinality(db) do
    IO.puts("\nPhase 4: High-cardinality label index performance")
    
    now = System.system_time(:millisecond)
    
    # Create 100k unique series via labels
    IO.puts("  Creating 100k unique series (combinations of labels)...")
    for series_i <- 1..100_000 do
      host = "host_#{series_i}"
      value = :rand.uniform() * 100.0
      Chronos.ingest(db, "memory.used", %{host: host}, value, now)
    end

    Benchee.run(
      %{
        "cardinality lookup — filter by single label" => fn ->
          host = "host_#{:rand.uniform(100_000)}"
          Chronos.Series.lookup_by_labels(%{host: host})
        end,
        "ingest into high-cardinality series" => fn ->
          host = "host_#{:rand.uniform(100_000)}"
          value = :rand.uniform() * 100.0
          Chronos.ingest(db, "memory.used", %{host: host}, value, System.system_time(:millisecond))
        end
      },
      parallel: 4,
      time: 5,
  ...
  defp measure_aggregations(db) do
    IO.puts("\nPhase 5: Aggregation functions (downsampling)")
    
    now = System.system_time(:millisecond)
    from = now - 86_400_000  # 1 day ago
    
    Benchee.run(
      %{
        "aggregation — :avg over 1h window" => fn ->
          host = "host_#{:rand.uniform(100)}"
          Chronos.query(db, "cpu.usage", %{host: host},
            from: from, to: now, aggregate: :avg, step: "1h")
        end,
        "aggregation — :max over 1h window" => fn ->
          host = "host_#{:rand.uniform(100)}"
          Chronos.query(db, "cpu.usage", %{host: host},
            from: from, to: now, aggregate: :max, step: "1h")
        end,
        "aggregation — :last (gap-fill)" => fn ->
          host = "host_#{:rand.uniform(100)}"
          Chronos.query(db, "cpu.usage", %{host: host},
            from: from, to: now, aggregate: :last, step: "5m", gap_fill: :fill_previous)
        end
      },
      parallel: 4,
      time: 5,
...
### Benchmark targets (v3 standard) — Compression + Ingest Focus

| Metric | Target | Notes |
|--------|--------|-------|
| **Ingest throughput** | 1M points/s | Single point with labels |
| **Ingest with batch (10 points)** | 500k batches/s | 5M points/s effective throughput |
| **Gorilla compression ratio (monotonic)** | 50–100:1 | Delta-of-delta + XOR on counters |
| **Gorilla compression ratio (random)** | 2–5:1 | Degrades for non-uniform data |
| **Gorilla encode latency (1000 samples)** | < 1 ms | Bitstring accumulation |
| **Gorilla decode latency (1000 samples)** | < 2 ms | Bitstring parsing |
| **Range query (1h, 5m step, single series)** | 10k queries/s | 12 aggregation windows |
| **Range query latency p50** | < 5 ms | Label index + chunk reads |
| **Range query latency p99** | < 20 ms | Multiple chunks, GC pause possible |
| **High-cardinality label lookup (100k series)** | 100k lookups/s | ETS bag intersection |
| **Aggregation (:avg, 1h window)** | 10k queries/s | Decompress + aggregate |

### Downsampling strategy (production-grade)

For retention policies:
- **Raw data**: 24h retention, 10-second resolution = 8,640 points/series/day
- **Hourly rollup**: 30 days × 1 point/hour = 720 points/series/month
- **Daily rollup**: 1 year × 1 point/day = 365 points/series/year

Downsampling must:
1. Run on completed hours only (not current hour being written)
2. Emit aggregate before deleting raw data
3. Use a separate "downsampler" process to avoid contention with ingest

Example: At 14:30, downsampler reads 13:00–14:00 raw data, computes hourly averages, writes to `cpu.usage.hourly`, then deletes raw 13:00–14:00.
