...
```elixir
# bench/searcher_bench.exs — Full-text search engine with BM25 ranking
# mix run bench/searcher_bench.exs

defmodule SearcherBench do
  @doc "Measures search throughput, BM25 scoring, and ranking quality."
  def run do
    {:ok, engine} = Searcher.Engine.start_link()
    
    IO.puts("=== Full-Text Search Benchmark (v3 standard) ===\n")

    # Build corpus: 100k documents
    IO.puts("Indexing 100,000 documents...")
    build_corpus(engine, 100_000)
    IO.puts("Index complete.\n")

    # Benchmark phases
    measure_search_throughput(engine)
    measure_ranking_quality(engine)
    measure_phrase_queries(engine)
    measure_boolean_queries(engine)
    measure_indexing_rate(engine)
  end

  defp build_corpus(engine, doc_count) do
    topics = ~w[machine learning deep neural network training model optimization classification regression]
    
    for i <- 1..doc_count do
      # Randomly pick topic keywords and add padding
      topic = Enum.random(topics)
      keywords = Enum.take_random(topics, 3) |> Enum.join(" ")
      padding = Enum.join(for(_ <- 1..(:rand.uniform(200)), do: "word"), " ")
      text = "#{topic} #{keywords} #{padding}"
      
      Searcher.Engine.index(engine, "doc_#{i}", text)
    end
  end

  defp measure_search_throughput(engine) do
    IO.puts("Phase 1: Single-term and multi-term BM25 search")
    
    latencies_single = []
    latencies_multi = []
    
    {_time, latencies_single} = :timer.tc(fn ->
      for _ <- 1..5000 do
        start = System.monotonic_time(:microsecond)
        Searcher.Engine.search(engine, "machine", scorer: :bm25, top_k: 10)
        elapsed = System.monotonic_time(:microsecond) - start
        [elapsed | latencies_single]
      end
    end)

    {_time, latencies_multi} = :timer.tc(fn ->
      for _ <- 1..5000 do
        start = System.monotonic_time(:microsecond)
        Searcher.Engine.search(engine, "machine learning", scorer: :bm25, top_k: 10)
        elapsed = System.monotonic_time(:microsecond) - start
        [elapsed | latencies_multi]
      end
    end)

    Benchee.run(
      %{
        "search — single term 'machine' (top 10)" => fn ->
          Searcher.Engine.search(engine, "machine", scorer: :bm25, top_k: 10)
        end,
        "search — 2-term 'machine learning' (top 10)" => fn ->
          Searcher.Engine.search(engine, "machine learning", scorer: :bm25, top_k: 10)
        end,
        "search — 3-term 'deep neural network' (top 10)" => fn ->
          Searcher.Engine.search(engine, "deep neural network", scorer: :bm25, top_k: 10)
        end
      },
      parallel: 8,
      time: 10,
    ...
    compute_percentiles("Single-term search latency", latencies_single)
    compute_percentiles("Multi-term search latency", latencies_multi)
  end

  defp measure_ranking_quality(engine) do
    IO.puts("\nPhase 2: BM25 ranking (short vs long documents)")
    
    # Clear and rebuild with controlled documents
    Searcher.Engine.index(engine, "short_doc", "machine learning is great " <> Enum.join(for(_ <- 1..5, do: "padding"), " "))
    Searcher.Engine.index(engine, "long_doc", "machine " <> Enum.join(for(_ <- 1..500, do: "padding"), " "))
    
    results = Searcher.Engine.search(engine, "machine", scorer: :bm25, top_k: 2)
    short_rank = Enum.find_index(results, fn {id, _score} -> id == "short_doc" end)
    long_rank = Enum.find_index(results, fn {id, _score} -> id == "long_doc" end)
    
    IO.puts("\nBM25 ranking test:")
    if short_rank == 0 do
      IO.puts("  ✓ Short document ranked higher (as expected)")
    else
      IO.puts("  ✗ Long document ranked higher (BM25 length normalization issue)")
    end
    
    case results do
      [{_id1, score1}, {_id2, score2} | _] ->
        IO.puts("  Score ratio (short/long): #{Float.round(score1 / score2, 2)}:1")
      _ ->
        :ok
    end
  end

  defp measure_phrase_queries(engine) do
    IO.puts("\nPhase 3: Phrase query latency (positional index)")
    ...
    Benchee.run(
      %{
        "phrase search — 'machine learning'" => fn ->
          Searcher.Engine.phrase_search(engine, "machine learning")
        end,
        "phrase search — 'deep neural network'" => fn ->
          Searcher.Engine.phrase_search(engine, "deep neural network")
        end
      },
      parallel: 4,
      time: 5,
    ...
    compute_percentiles("Phrase query latency", latencies)
  end

  defp measure_boolean_queries(engine) do
    IO.puts("\nPhase 4: Boolean AND/OR query intersection")
    
    Benchee.run(
      %{
        "boolean AND — 'machine AND learning'" => fn ->
          Searcher.Engine.search(engine, "machine learning", scorer: :bm25, top_k: 10)
        end,
        "boolean OR — 'machine OR neural'" => fn ->
          terms = Searcher.Pipeline.process("machine neural") |> Enum.map(fn {stem, _} -> stem end) |> Enum.uniq()
          doc_scores = 
            Enum.flat_map(terms, fn term ->
              postings = Searcher.InvertedIndex.postings(term)
              Enum.map(postings, fn {doc_id, tf, _positions} ->
                {total_docs, avg_len} = Searcher.InvertedIndex.stats()
                score = Searcher.Scorer.bm25(tf, length(postings), Searcher.InvertedIndex.doc_length(doc_id), avg_len, total_docs)
                {doc_id, score}
              end)
            end)
            |> Enum.group_by(fn {doc_id, _} -> doc_id end, fn {_, score} -> score end)
            |> Enum.map(fn {doc_id, scores} -> {doc_id, Enum.sum(scores)} end)
  ...
  defp measure_indexing_rate(engine) do
    IO.puts("\nPhase 5: Indexing rate (documents per second)")
    
    Benchee.run(
      %{
        "index — single document (100 words)" => fn ->
          doc_id = "doc_#{:rand.uniform(1_000_000)}"
          text = Enum.join(for(_ <- 1..100, do: "word"), " ")
          Searcher.Engine.index(engine, doc_id, text)
        end,
        "index — batch 10 documents" => fn ->
          for i <- 1..10 do
            doc_id = "doc_#{:rand.uniform(1_000_000)}_#{i}"
            text = Enum.join(for(_ <- 1..100, do: "word"), " ")
            Searcher.Engine.index(engine, doc_id, text)
          end
        end
...
### Benchmark targets (v3 standard) — Ranking + Relevance Focus

| Metric | Target | Notes |
|--------|--------|-------|
| **Single-term search (1M docs)** | 100k queries/s | Posting list lookup + BM25 scoring |
| **Single-term query latency p50** | < 1 ms | Hot posting lists in ETS |
| **Single-term query latency p99** | < 50 ms | Cold posting lists, GC pause |
| **Multi-term AND (2 terms)** | 50k queries/s | Posting list intersection |
| **Multi-term AND latency p99** | < 10 ms | Intersection of two posting lists |
| **Multi-term AND (3+ terms)** | 10k queries/s | More intersections; selectivity drops |
| **Phrase search latency p99** | < 100 ms | Positional filtering on matching docs |
| **Boolean OR (union)** | 50k queries/s | Union of posting lists; no filtering |
| **Indexing rate** | 10k docs/s | Tokenize + stem + update inverted index |
| **BM25 length normalization** | 100% correct | Short docs rank above long docs on same term |
| **Phrase query accuracy** | 100% | Only consecutive positions counted |
| **Memory per term** | < 1 KB | Posting list + ETS overhead |

### Inverted Index Optimization Strategies

1. **Posting list compression**: Store document IDs as variable-length integers (VInt). Cut posting list size by 50–70% without affecting query speed.

2. **Skip lists in posting lists**: Add skip pointers every 128 documents. For AND queries, jump over document IDs that can't possibly match, reducing comparisons from O(N1 + N2) to O(N1 + log N2).

3. **Term frequency limits**: Cap TF contributions in BM25 at k1 (default 1.5). Prevents keyword-stuffing attacks and bounds scoring latency.

4. **Incremental indexing**: On document updates, recompute only affected terms, not the entire document. Requires a "tombstone" marker on old versions.

### BM25 Tuning (production)

Default parameters (k1=1.5, b=0.75) work well for most English text corpora. Tuning guide:

- **k1 up to 2.0**: Increase TF contribution for longer queries (5+ terms). Helps queries on specialized vocabularies.
- **b up to 1.0**: Increase length penalty if corpus has wildly variable document lengths (web pages vs abstracts).
- **b down to 0.5**: Decrease length penalty if corpus is uniform in length or if short documents are naturally relevant (tweet search).

Never tune on your test corpus alone—use a held-out evaluation set (Mean Average Precision, NDCG@10).
