...
**Objective**: Generate `--sup` skeleton so MVCC storage, lock manager, and query planner can crash-restart independently.
...
**Objective**: Rely on ETS for row storage, no third-party DB client; only `:benchee` for measurements to avoid license and audit friction.
...
**Objective**: Version rows by monotonic txn id so readers never block writers and snapshot isolation falls out for free.
...
**Objective**: Pick index scan vs seq scan via cardinality estimates, because the wrong plan turns microseconds into seconds on large tables.
...
**Objective**: Track lock ownership in a wait-for graph so cycles are detected and deadlocks abort the youngest txn, not the system.
...
**Objective**: Wrap `begin/commit/rollback` around MVCC and locks so callers write SQL semantics, never touching txn internals.
...
**Objective**: Pin snapshot isolation and deadlock detection invariants so MVCC refactors cannot silently regress to dirty reads.
...
**Objective**: Force `--seed 0` so concurrent txn tests deterministically replay deadlock cycles across runs.
...
**Objective**: Drive 1M-row indexed selects and concurrent txns through Benchee so MVCC version-chain walks expose their cost before the optimizer masks it.
...
```elixir
# bench/memdb_bench.exs — In-memory database with MVCC snapshot isolation
# mix run bench/memdb_bench.exs

defmodule MemdbBench do
  @doc "Measures MVCC throughput and isolation latencies."
  def run do
    {:ok, _db_supervisor} = Memdb.Application.start(:normal, [])
    db = Memdb.Database
    
    IO.puts("=== In-Memory Database Benchmark (v3 standard) ===\n")

    # Setup: create table with 1M rows
    IO.puts("Setting up: populating 1M rows...")
    Memdb.Database.create_table(db, :users, [
    ...
    for i <- 1..1_000_000 do
      Memdb.Database.insert(db, :users, %{
        id: i,
        name: "user_#{i}",
        email: "user_#{i}@example.com",
        balance: :rand.uniform() * 10_000.0
      })
    end
    ...
    # Benchmark phases
    measure_point_reads(db)
    measure_range_scans(db)
    measure_point_writes(db)
    measure_snapshot_isolation(db)
    measure_deadlock_detection(db)
  end

  defp measure_point_reads(db) do
    IO.puts("Phase 1: Point reads (MVCC snapshot isolation)")
    latencies = []

    {_time, latencies} = :timer.tc(fn ->
      for _ <- 1..100_000 do
        start = System.monotonic_time(:microsecond)
        {:ok, txn} = Memdb.Database.begin(db)
        id = :rand.uniform(1_000_000)
        {:ok, [row]} = Memdb.Database.select(db, :users, [id: id], txn_id: txn)
        Memdb.Database.commit(db, txn)
    ...
    Benchee.run(
      %{
        "point read (MVCC snapshot)" => fn ->
          {:ok, txn} = Memdb.Database.begin(db)
          id = :rand.uniform(1_000_000)
          {:ok, _row} = Memdb.Database.select(db, :users, [id: id], txn_id: txn)
          Memdb.Database.commit(db, txn)
        end
      },
      parallel: 8,
    ...
    compute_percentiles("Point read latency (snapshot)", latencies)
  end

  defp measure_range_scans(db) do
    IO.puts("\nPhase 2: Range scans (sequential vs B-tree index)")
    
    Benchee.run(
      %{
        "range scan [id=1..1000] (full table)" => fn ->
          {:ok, txn} = Memdb.Database.begin(db)
          {:ok, rows} = Memdb.Database.select(db, :users, [], txn_id: txn)
          Enum.filter(rows, fn row -> row.id >= 1 and row.id <= 1_000 end)
          Memdb.Database.commit(db, txn)
        end,
        "range scan [balance > 5000] (B-tree on indexed column)" => fn ->
          {:ok, txn} = Memdb.Database.begin(db)
          {:ok, rows} = Memdb.Database.select(db, :users, [balance: {:>, 5_000.0}], txn_id: txn)
          rows
          Memdb.Database.commit(db, txn)
        end
      },
  ...
  defp measure_point_writes(db) do
    IO.puts("\nPhase 3: Point writes (MVCC new versions)")
    
    Benchee.run(
      %{
        "insert (new row version)" => fn ->
          {:ok, txn} = Memdb.Database.begin(db)
          id = :rand.uniform(1_000_000) + 1_000_000
          Memdb.Database.insert(db, :users, %{
            id: id,
            name: "new_user_#{id}",
            email: "new_#{id}@example.com",
            balance: :rand.uniform() * 10_000.0
          }, txn_id: txn)
          Memdb.Database.commit(db, txn)
        end,
        "update (create expired version)" => fn ->
          {:ok, txn} = Memdb.Database.begin(db)
          id = :rand.uniform(100_000)
          Memdb.Database.update(db, :users, [id: id], %{balance: :rand.uniform() * 10_000.0}, txn_id: txn)
          Memdb.Database.commit(db, txn)
        end,
        "delete (mark as expired)" => fn ->
          {:ok, txn} = Memdb.Database.begin(db)
          id = :rand.uniform(1_000_000) + 1_000_000
          Memdb.Database.delete(db, :users, [id: id], txn_id: txn)
          Memdb.Database.commit(db, txn)
        end
      },
      parallel: 4,
  ...
  defp measure_snapshot_isolation(db) do
    IO.puts("\nPhase 4: Snapshot isolation under concurrent read/write")
    
    # Spawn writer that modifies rows continuously
    spawn(fn ->
      for i <- 1..5_000 do
        {:ok, txn} = Memdb.Database.begin(db)
        Memdb.Database.update(db, :users, [id: 1], %{balance: i * 100.0}, txn_id: txn)
        Memdb.Database.commit(db, txn)
      end
    end)
    ...
    Benchee.run(
      %{
        "read while writer modifies (snapshot isolation)" => fn ->
          {:ok, txn} = Memdb.Database.begin(db)
          {:ok, [row]} = Memdb.Database.select(db, :users, [id: 1], txn_id: txn)
          # Snapshot should always see the same balance within the same txn
          Memdb.Database.commit(db, txn)
          row.balance
        end
      },
  ...
  defp measure_deadlock_detection(db) do
    IO.puts("\nPhase 5: Deadlock detection latency")
    
    Benchee.run(
      %{
        "detect deadlock cycle (DFS on wait-for graph)" => fn ->
          {:ok, txn1} = Memdb.Database.begin(db)
          {:ok, txn2} = Memdb.Database.begin(db)
          
          # Lock row 1 in txn1, row 2 in txn2
          Memdb.Database.update(db, :users, [id: 1], %{balance: 100.0}, txn_id: txn1)
          Memdb.Database.update(db, :users, [id: 2], %{balance: 200.0}, txn_id: txn2)
          
          # Try cross-lock (deadlock)
          case Memdb.Database.update(db, :users, [id: 2], %{balance: 300.0}, txn_id: txn1) do
            :ok -> Memdb.Database.commit(db, txn1)
            {:error, :deadlock} -> Memdb.Database.rollback(db, txn1)
          end
          
          Memdb.Database.rollback(db, txn2)
        end
      },
      parallel: 2,
      time: 5,
...
### Benchmark targets (v3 standard) — MVCC + Isolation Focus

| Metric | Target | Notes |
|--------|--------|-------|
| **Point read (MVCC snapshot)** | 1M ops/s | No locks, snapshot at XID |
| **Point read latency p50** | < 1 µs | In-memory ETS lookup + visibility check |
| **Point read latency p99** | < 50 µs | Includes occasional version chain scan |
| **Point read latency p999** | < 500 µs | GC running or many versions per row |
| **Insert (new version)** | 200k ops/s | WAL write + memtable append |
| **Update (create expired version)** | 100k ops/s | Old version + new version in ETS |
| **Delete (mark expired)** | 100k ops/s | Logical delete (no physical removal until GC) |
| **Range scan (10k rows)** | 50k scans/s | Full table scan (no index on this range) |
| **Range scan with B-tree index** | 500k ops/s | O(log N) seek + O(K) fetch K results |
| **Snapshot isolation consistency** | 100% | Same balance read twice in same txn |
| **Deadlock detection latency** | < 10 ms | DFS on wait-for graph per 100ms timer |
| **GC throughput** | 1M versions/s | Delete expired, below horizon XID |

### Comparison to 2PL (Two-Phase Locking)

| Aspect | MVCC (your impl) | 2PL |
|--------|-----------------|-----|
| Read throughput | 1M/s (no locks) | 100k/s (acquires locks) |
| Write throughput | 100k/s (new versions) | 50k/s (in-place update + log) |
| Latency p99 | < 50 µs | < 500 µs (lock wait) |
| Memory overhead | High (versions) | Low (single version) |
| GC pauses | Yes (version cleanup) | No (immediate reuse) |
| Phantom reads | Possible | Prevented (range locks) |
| Deadlock detection | Required | Yes, 2PL has cycles too |

MVCC wins on read throughput and write concurrency at the cost of memory and GC complexity.
