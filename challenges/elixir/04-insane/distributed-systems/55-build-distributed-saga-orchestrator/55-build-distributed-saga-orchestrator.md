...
**Objective**: Pair every forward action with a compensation at declaration time so no step can ship without a rollback path.
...
**Objective**: Persist each transition before side effects fire so recovery can replay intent without observing unacknowledged outcomes.
...
**Objective**: Sequence step execution so at-least-once retries stay idempotent and partial progress never leaks across saga boundaries.
...
**Objective**: Invoke compensations in strict reverse order so rollback respects causal dependencies between already-applied forward effects.
...
**Objective**: Recover in-flight state from durable storage after a crash or restart.
...
def main do
  IO.puts("[SagaEngine.Orchestrator] GenServer demo")
  :ok
end
