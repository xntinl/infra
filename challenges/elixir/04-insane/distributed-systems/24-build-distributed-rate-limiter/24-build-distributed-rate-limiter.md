...
**Objective**: Split bucket algorithm, window aggregation, and cluster quorum into modules so each throttling invariant stays testable standalone.
...
**Objective**: Pin only benchee and stream_data so token accounting and quorum logic never lean on hidden rate-limit libraries.
...
**Objective**: Reconcile per-node counters through quorum reads so cluster-wide limits stay globally correct under partition and hotspot keys.
...
**Objective**: Shard keys consistently across nodes so identical client ids always aggregate on the same quorum replica set.
...
**Objective**: Lock down refill correctness, global-limit enforcement, and partition tolerance as executable specs the implementation cannot edit.
...
**Objective**: Run the suite deterministically so quorum timing and token-refill races surface as reproducible traces rather than flaky assertions.
...
def main do
  IO.puts("[Throttlex.TokenBucketTest] GenServer demo")
  :ok
end
