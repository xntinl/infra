#!/usr/bin/env elixir
# Agent-7 Level 2: Add script/main.exs blocks
# Correctly finds 210+ files in 03-avanzado and all in 04-insane

defmodule Agent7Working do
  ...
  def run do
    ...
    final_message = "Agent-7 (03-avanzado 210+ + 04-insane): #{length(all_files)} archivos procesados, #{added} script blocks agregados"
    log(final_message)
  ...
  # Extract number from directory name: /XXX-name/filename.md -> XXX
  defp extract_dir_number(file_path) do
    parent_dir = Path.dirname(file_path) |> Path.basename()
      ...
      if String.contains?(content, "### `script/main.exs`") do
          ...
          {_pre, ""} ->
            ...
            case generate_script(content) do
              ...
              new_script ->
                ...
                log("UPDATED (empty block): #{Path.basename(file_path)}")
                :added
            end
        ...
        case generate_script(content) do
          ...
          new_script ->
            ...
            File.write!(file_path, new_content)
            log("ADDED (new section): #{Path.basename(file_path)}")
            :added
        end
      end
    rescue
      e ->
        log("ERROR in #{Path.basename(file_path)}: #{inspect(e)}")
        :skipped
    else
      result -> result
    end
