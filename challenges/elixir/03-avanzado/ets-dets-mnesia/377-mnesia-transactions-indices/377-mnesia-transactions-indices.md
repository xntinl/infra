...
defmodule AccountLedger.Schema do
  @moduledoc "Creates Mnesia tables at boot. Idempotent."
  ...
  def setup do
    ...
    create_table(:accounts,
      attributes: [:id, :balance, :owner],
      type: :set
    )

    create_table(:transfers,
      attributes: [:id, :from, :to, :amount, :reference_id, :at],
      type: :set,
      index: [:reference_id]
    )

    :ok = :mnesia.wait_for_tables([:accounts, :transfers], 5_000)
  end

  defp create_table(name, opts) do
    case :mnesia.create_table(name, opts) do
      {:atomic, :ok} ->
        Logger.info("mnesia: created #{name}")

      {:aborted, {:already_exists, ^name}} ->
        Logger.info("mnesia: #{name} already exists")

      {:aborted, reason} ->
        raise "mnesia table #{name} failed: #{inspect(reason)}"
    end
  end
end
```
...
defmodule AccountLedger.Ledger do
  @moduledoc """
  Ledger operations.

  * `open_account/2`         creates an account with an initial balance.
  * `transfer/4`             atomic money movement between two accounts, idempotent on `reference_id`.
  * `balance/1`              dirty read — fast, eventually consistent.
  * `balance_strict/1`       transactional read — slower, always consistent.
  * `find_by_reference/1`    secondary-index lookup.
  """

  def open_account(id, initial_balance, owner) when initial_balance >= 0 do
    tx = fn ->
      :mnesia.write({:accounts, id, initial_balance, owner})
    end
  ...
  @spec transfer(String.t(), String.t(), non_neg_integer(), String.t()) ::
          {:ok, String.t()} | {:error, term()}
  def transfer(from, to, amount, reference_id) when amount > 0 do
    tx = fn ->
      case :mnesia.index_read(:transfers, reference_id, :reference_id) do
        [existing] ->
          # Idempotency hit — return the previously recorded transfer id
          {:ok, elem(existing, 1)}

        [] ->
          [{:accounts, ^from, balance_from, from_owner}] = :mnesia.read({:accounts, from})
          [{:accounts, ^to, balance_to, to_owner}] = :mnesia.read({:accounts, to})

          if balance_from < amount do
            :mnesia.abort(:insufficient_funds)
          end

          :mnesia.write({:accounts, from, balance_from - amount, from_owner})
          :mnesia.write({:accounts, to, balance_to + amount, to_owner})

          transfer_id = "tx_" <> Base.url_encode64(:crypto.strong_rand_bytes(8), padding: false)
          now = System.system_time(:millisecond)
          :mnesia.write({:transfers, transfer_id, from, to, amount, reference_id, now})
  ...
  def balance(id) do
    case :mnesia.dirty_read({:accounts, id}) do
      [{:accounts, ^id, bal, _owner}] -> {:ok, bal}
      [] -> :not_found
    end
  end

  def balance_strict(id) do
    tx = fn ->
  ...
  def find_by_reference(reference_id) do
    tx = fn ->
      case :mnesia.index_read(:transfers, reference_id, :reference_id) do
        [t] -> {:ok, t}
        [] -> :not_found
...
defmodule AccountLedger.Application do
  use Application

  @impl true
  def start(_type, _args) do
    AccountLedger.Schema.setup()
    Supervisor.start_link([], strategy: :one_for_one, name: AccountLedger.Supervisor)
  end
end
```
