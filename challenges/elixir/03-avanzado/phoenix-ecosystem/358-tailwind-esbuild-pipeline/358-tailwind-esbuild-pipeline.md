...
config :admin_ui, AdminUiWeb.Endpoint,
  url: [host: "localhost"],
  render_errors: [formats: [html: AdminUiWeb.ErrorHTML], layout: false],
  pubsub_server: AdminUi.PubSub,
...
config :admin_ui, AdminUiWeb.Endpoint,
  ...
  live_reload: [
    patterns: [
      ...
      ~r"lib/admin_ui_web/(controllers|live|components)/.*(ex|heex)$"
    ]
  ]
```
...
/* Custom design tokens can live here */
.btn {
  @apply inline-flex items-center px-4 py-2 rounded-md bg-brand text-white hover:opacity-90;
}
```
...
Build with `MIX_ENV=prod mix assets.deploy && MIX_ENV=prod mix release`.
