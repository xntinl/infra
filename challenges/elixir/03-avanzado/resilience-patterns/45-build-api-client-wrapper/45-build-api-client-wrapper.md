...
Build a resilient HTTP client layer for an API gateway that proxies requests to upstream
services. The client implements a circuit breaker to isolate failing upstreams, retry logic
with exponential backoff that respects `Retry-After` headers, and telemetry events for
observability.
...
The payments service has an SLA of 99.9% uptime, but occasionally becomes slow (>5s
responses) rather than outright down. A slow upstream means all gateway requests pile up in
connection pools, exhausting file descriptors. The client needs:
...
the first wave of failures and rejects subsequent requests immediately.
...
A GenServer holding circuit state serializes all access. For a high-traffic gateway, this
becomes the bottleneck.

ETS with `:set, :public, read_concurrency: true` allows concurrent reads without any
process overhead. Only state transitions require coordinated writes.
...
The circuit breaker stores per-host state in ETS for lock-free reads.
...
Retry logic wraps any function call with exponential backoff. It distinguishes retryable
server errors (5xx, 429) from non-retryable client errors (4xx) and respects `Retry-After`
headers from rate-limited responses.
...
Telemetry handlers convert raw events into structured log lines.
...
A telemetry event with `status: 0` when there's no HTTP response is still valuable -- it
tells the ops team the request never reached the service. Always emit telemetry regardless
of outcome.
