...
# Demonstrate custom child specs
IO.puts("=== Custom Child Spec Demo ===")

# Test child_spec with custom options
spec1 = CustomChildSpec.DurableWorker.child_spec(id: :log_writer, shutdown: 30_000)
spec2 = CustomChildSpec.DurableWorker.child_spec(id: :cache, restart: :transient)

assert spec1.id == :log_writer
assert spec1.shutdown == 30_000
assert spec1.restart == :permanent
assert spec1.type == :worker

assert spec2.id == :cache
assert spec2.restart == :transient
assert spec2.shutdown == 5_000

IO.puts("Child spec 1 (log_writer): id=#{spec1.id}, shutdown=#{spec1.shutdown}")
IO.puts("Child spec 2 (cache): id=#{spec2.id}, restart=#{spec2.restart}")

# Test that missing :id raises error
assert_raise KeyError, fn ->
  CustomChildSpec.DurableWorker.child_spec([])
end

IO.puts("Correctly raised KeyError for missing :id")

# Test starting workers with custom specs
{:ok, _pid1} = CustomChildSpec.DurableWorker.start_link(id: :writer)
{:ok, _pid2} = CustomChildSpec.DurableWorker.start_link(id: :scratch, restart: :transient)

assert CustomChildSpec.DurableWorker.get_id(:writer) == :writer
assert CustomChildSpec.DurableWorker.get_id(:scratch) == :scratch

IO.puts("Started two workers with custom specs")
IO.puts("Worker 1 ID: #{inspect(CustomChildSpec.DurableWorker.get_id(:writer))}")
