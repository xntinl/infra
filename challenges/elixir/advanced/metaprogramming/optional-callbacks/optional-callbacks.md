...
defmodule OptionalCallbacks do
  ...
  Mandatory callbacks force users to write trivial stubs just to satisfy the compiler. Optional callbacks let the framework ask `function_exported?/3` once at init and pick the....
  """
end
```
### `lib/optional_callbacks/psp.ex`

**Objective**: Define @callback list and @optional_callbacks list so PSPs skip unneeded stubs at compile time.

### `mix.exs`
```elixir
defmodule OptionalCallbacks.MixProject do
  @moduledoc """
  Ejercicio: Optional Callbacks and Runtime `ensure_loaded?`.
...
```
### `lib/optional_callbacks/psps/stripe.ex`
...
```
### `lib/optional_callbacks/psps/wire_transfer.ex`
...
```
### `lib/optional_callbacks/psps/paypal.ex`
...
```
### `lib/optional_callbacks/orchestrator.ex`
...
defmodule OptionalCallbacksTest do
  ...
  doctest OptionalCallbacks.Orchestrator
...
## Why Optional Callbacks and Runtime `ensure_loaded?` matters

Mastering **Optional Callbacks and Runtime `ensure_loaded?`** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
