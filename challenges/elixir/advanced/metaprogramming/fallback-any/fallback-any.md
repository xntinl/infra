...
```elixir
defmodule FallbackAny do
  @moduledoc """
  `@fallback_to_any true` — Deep Dive and Performance Cost.

  Requiring `defimpl` for every type means every new struct in every consumer is a potential production crash. `@fallback_to_any` routes unknown types to a single `defimpl Protocol,....
  """
end
```
### `lib/fallback_any/with_fallback.ex`

**Objective**: Define protocol with @fallback_to_any true and defimpl for: Any so unknown types route to term_to_binary safely.

### `mix.exs`
```elixir
defmodule FallbackAny.MixProject do
  use Mix.Project
...
```
### `lib/fallback_any/without_fallback.ex`
...
```
### `lib/fallback_any/bench_driver.ex`
...
defmodule FallbackAnyTest do
  ...
  doctest FallbackAny.BenchDriver
  ...
  describe "WithFallback (has Any)" do
    ...
    test "encodes unknown struct via Any impl" do
      ...
      assert :erlang.binary_to_term(bin, [:safe]) == %Weird{x: 1}
    end
...
defmodule Main do
  def main do
      ...
      defimpl WithFallback, for: String do
        def process_value(s), do: {:string, s}
      end

      # Fallback implementation for any type
      defimpl WithFallback, for: Any do
        def process_value(data), do: {:any, data}
      end

      # Benchmark dispatch
      t0 = System.monotonic_time(:microsecond)
      result1 = WithFallback.process_value("hello")
      t1 = System.monotonic_time(:microsecond)

      result2 = WithFallback.process_value(42)
...
## Why `@fallback_to_any true` — Deep Dive and Performance Cost matters

Mastering **`@fallback_to_any true` — Deep Dive and Performance Cost** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
