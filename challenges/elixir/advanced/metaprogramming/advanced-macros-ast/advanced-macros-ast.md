...
### `mix.exs`
```elixir
defmodule AdvancedMacrosAst.MixProject do
  use Mix.Project

  def project do
    [
      app: :advanced_macros_ast,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule AstSurgeon.MixProject do
  ...
  def project do
    [app: :ast_surgeon, version: "0.1.0", elixir: "~> 1.19", deps: deps()]
  end
...
```elixir
defmodule AstSurgeon do
  @moduledoc """
  Advanced Macros and AST Surgery.

  String-built code loses all Elixir's scoping, macro hygiene, and source-location info. AST code keeps each node anchored to an original source location, so a broken macro points....
  """
end
```

### `lib/ast_surgeon/traced_http.ex`
...
## Why Advanced Macros and AST Surgery matters

Mastering **Advanced Macros and AST Surgery** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
