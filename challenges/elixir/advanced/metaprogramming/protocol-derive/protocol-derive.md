...
### `lib/protocol_derive.ex`

```elixir
defmodule ProtocolDerive do
  @moduledoc """
  Protocol `@derive` — Consuming and Implementing.

  Hand implementations become N copies of the same `defimpl`. `@derive` calls a single code-generating function per struct at compile time, keeping the protocol's contract in one....
  """
end
```

### `lib/protocol_derive/audit_encoder.ex`
...
### `mix.exs`
```elixir
defmodule ProtocolDerive.MixProject do
  use Mix.Project
...
### `lib/protocol_derive/helpers.ex`
...
### `lib/protocol_derive/schemas.ex`
...
defmodule ProtocolDeriveTest do
  ...
  doctest ProtocolDerive.Schemas
...
## Why Protocol `@derive` — Consuming and Implementing matters

Mastering **Protocol `@derive` — Consuming and Implementing** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
