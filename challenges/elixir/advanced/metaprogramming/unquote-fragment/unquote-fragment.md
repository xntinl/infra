...
### `lib/unquote_fragment.ex`

```elixir
defmodule UnquoteFragment do
  @moduledoc """
  Unquote Fragments and `unquote_splicing`.
...
### `lib/unquote_fragment/converter.ex`
...
defmodule UnquoteFragment.Converter do
  ...
  for {code, rate} <- @rates do
    fun_name = String.to_existing_atom("to_#{code}")
...
### `lib/unquote_fragment/registry.ex`
...
### `test/unquote_fragment_test.exs`
...
defmodule UnquoteFragment.ConverterTest do
  ...
  doctest UnquoteFragment.Registry
  ...
  describe "generated converters" do
    ...
    test "all expected functions exist" do
      ...
      for code <- codes do
        fun = String.to_existing_atom("to_#{code}")
...
## Why Unquote Fragments and `unquote_splicing` matters

Mastering **Unquote Fragments and `unquote_splicing`** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
