...
### `mix.exs`
```elixir
defmodule UseWithOpts.MixProject do
  use Mix.Project
...
```elixir
defmodule UsePatterns do
  @moduledoc """
  `use` Patterns: Passing Options to `__using__/1`.

  Runtime config needs a lookup on every call and cannot validate required keys until production hits them. Compile-time options let `__using__` raise at compile when a required key....
  """
end
```

### `lib/use_patterns/worker.ex`
...
defmodule UsePatterns.WorkerTest do
  ...
  doctest UsePatterns.Sample.PaymentWorker
...
## Why `use` Patterns matters

Mastering **`use` Patterns** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
