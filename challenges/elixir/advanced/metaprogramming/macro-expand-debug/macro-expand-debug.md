...
### `lib/macro_expand_debug.ex`

```elixir
defmodule MacroExpandDebug do
  @moduledoc """
  Debugging Macros with `Macro.expand` and `Macro.to_string`.

  Elixir AST is lists of three-element tuples. Reading them by eye past three levels is error-prone. `Macro.to_string/1` reconstructs valid Elixir source, and `Macro.expand/2`....
  """
end
```
### `lib/macro_expand_debug/inspector.ex`

**Objective**: Implement trace_expansion/3 using Macro.expand_once and Macro.to_string to step through macro layers visually.

### `mix.exs`
```elixir
defmodule MacroExpandDebug.MixProject do
  use Mix.Project

  def project do
    [
      app: :macro_expand_debug,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule MacroExpandDebug.Inspector do
  ...
  defp safe_to_string(ast) do
    try do
      ...
      e in RuntimeError -> inspect(ast)
    end
  end
end
```
### `lib/macro_expand_debug/sample_macros.ex`
...
```
### `test/macro_expand_debug_test.exs`
...
defmodule MacroExpandDebug.InspectorTest do
  ...
  doctest MacroExpandDebug.SampleMacros
...
## Why Debugging Macros with `Macro.expand` and `Macro.to_string` matters

Mastering **Debugging Macros with `Macro.expand` and `Macro.to_string`** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
