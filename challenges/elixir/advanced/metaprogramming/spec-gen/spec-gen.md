...
### `mix.exs`
```elixir
defmodule SpecGen.MixProject do
  use Mix.Project
...
defmodule SpecGen do
  ...
  defp parse_head({name, _meta, args}, caller) when is_atom(name) and is_list(args) do
    ...
    vars =
      types
      |> Enum.with_index()
      |> Enum.map(fn {t, i} -> Macro.var(String.to_existing_atom("#{t}#{i}"), __MODULE__) end)
...
## Why Generating `@spec` from Annotations matters

Mastering **Generating `@spec` from Annotations** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
