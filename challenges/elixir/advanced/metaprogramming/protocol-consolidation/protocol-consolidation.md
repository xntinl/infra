...
## Why protocol consolidation and dispatch performance matters
...
**Option A — leave consolidation at defaults (`consolidate_protocols: Mix.env() != :test`)**
- Pros: zero config; dev iteration stays fast because the compiler skips the consolidation pass.
- Cons: easy to ship a release with protocols *not* consolidated because `MIX_ENV=prod` was forgotten in CI, or a dev dependency flipped the flag; the slowdown is invisible until profiled.

**Option B — always consolidate in release, fail the build when a release is cut without it** (chosen)
- Pros: the dispatch cost you benchmarked is the dispatch cost production pays; no "works on my machine" divergence between dev and prod protocol throughput.
- Cons: CI must run the consolidation assertion, and devs who `iex -S mix` against a consolidated project lose the ability to hot-reload new `defimpl` clauses without a recompile.

→ Chose **B** because in the profile that motivated this exercise the 15% CPU spent on dispatch was a direct result of a CI misconfiguration that shipped a release with `consolidate_protocols: false`. Making the invariant explicit in code closes the door on that failure mode.
...
```
### `lib/proto_consolidation.ex`

```elixir
defmodule ProtoConsolidation do
  @moduledoc """
  Protocol Consolidation and Dispatch Performance.
...
```
### `lib/proto_consolidation/encoder.ex`
...
```
### `lib/proto_consolidation/impls.ex`
...
```
### `lib/proto_consolidation/runner.ex`
...
```
### `test/proto_consolidation_test.exs`
...
defmodule ProtoConsolidation.EncoderTest do
  ...
  doctest ProtoConsolidation.Runner
...
defmodule Main do
  def main do
      ...
      defimpl TestProto, for: String do
        def process_value(str), do: String.upcase(str)
      end

      defimpl TestProto, for: Integer do
        def process_value(num), do: num * 2
      end

      # Measure dispatch performance
      t0 = System.monotonic_time(:microsecond)
      result1 = TestProto.process_value("hello")
      t1 = System.monotonic_time(:microsecond)

      result2 = TestProto.process_value(42)
