...
### `lib/binary_leak_lab.ex`

```elixir
defmodule BinaryLeakLab.Leaker do
  @moduledoc """
  Ejercicio: Binary Reference Counting and Memory Leaks.
  ...
  @doc "Returns ingest result from blob."
...
### `test/binary_leak_lab_test.exs`

```elixir
defmodule BinaryLeakLab.LeakTest do
  use ExUnit.Case, async: true
  doctest BinaryLeakLab.Leaker
...
defmodule Main do
  @moduledoc """
  Demonstration entry point for Binary Reference Counting and Memory Leaks.

  Run with: `mix run script/main.exs`
  """

  def main do
    IO.puts("=== Binary Reference Counting and Memory Leaks ===")
    ...
    results =
      Enum.map(payloads, fn payload ->
        case BinaryLeakLab.run(payload) do
    ...
    {us, _} =
      :timer.tc(fn ->
        for _ <- 1..1_000, do: BinaryLeakLab.run(:bench)
      end)
