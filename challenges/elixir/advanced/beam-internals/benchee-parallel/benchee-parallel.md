...
defmodule BencheeParallel.GenserverCounter do
  ...
  @doc "Returns incr result."
  @spec incr() :: :ok
  def incr, do: GenServer.call(__MODULE__, :incr)

  @doc "Returns value result."
...
### `test/benchee_parallel_test.exs`

```elixir
defmodule BencheeParallel.CountersTest do
  use ExUnit.Case, async: true
  doctest BencheeParallel.Application
...
defmodule Main do
  @moduledoc """
  Demonstration entry point for Benchee Parallel: Measuring Contention.

  Run with: `mix run script/main.exs`
  """

  def main do
    IO.puts("=== Benchee Parallel: Measuring Contention ===")
    ...
    results =
      Enum.map(payloads, fn payload ->
        case BencheeParallel.run(payload) do
    ...
    {us, _} =
      :timer.tc(fn ->
        for _ <- 1..1_000, do: BencheeParallel.run(:bench)
      end)
