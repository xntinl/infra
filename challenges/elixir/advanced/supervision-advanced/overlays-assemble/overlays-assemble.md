...
## Why release overlays and custom assemble steps matters
...
### `lib/overlays_assemble.ex`

```elixir
defmodule OverlaysAssemble do
  @moduledoc """
  Release Overlays and Custom Assemble Steps.
...
### `lib/overlays_assemble/application.ex`
...
### `lib/overlays_assemble/service.ex`
...
defmodule OverlaysAssemble.ReleaseLayoutTest do
  ...
  doctest OverlaysAssemble.Service
...
defmodule Main do
  def main do
      ...
      assert String.version_content != [], "VERSION should be stamped"
...
### `test/overlays_assemble_test.exs`

```elixir
defmodule OverlaysAssembleTest do
  use ExUnit.Case, async: true

  doctest OverlaysAssemble

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert OverlaysAssemble.run(:noop) == :ok
    end
  end
end
```
