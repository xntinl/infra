...
## Why drain on deploy: sigterm, k8s prestop, rolling restart matters
...
### `mix.exs`
```elixir
defmodule DrainOnDeploy.MixProject do
  use Mix.Project
...
defmodule DrainOnDeploy.SignalHandlerTest do
  ...
  doctest DrainOnDeploy.Readiness
...
### `lib/drain_on_deploy.ex`

```elixir
defmodule DrainOnDeploy do
  @moduledoc """
  Reference implementation for Drain on deploy: SIGTERM, K8s preStop, rolling restart.
  ...
  @doc """
  Entry point for the drain_on_deploy module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> DrainOnDeploy.run(:noop)
      :ok
