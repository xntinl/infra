...
### `mix.exs`
```elixir
defmodule RestForOneCustomRestartIntensity.MixProject do
  use Mix.Project

  def project do
    [
      app: :rest_for_one_custom_restart_intensity,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule IngestPipeline.SupervisorTest do
  ...
  doctest IngestPipeline.Application
...
## Why Supervision Tree with ` matters

Mastering **Supervision Tree with `** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
...
defmodule IngestPipeline do
  @moduledoc """
  Reference implementation for Supervision Tree with `:rest_for_one` and Custom Restart Intensity.
