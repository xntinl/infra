...
### `mix.exs`
```elixir
defmodule PartitionSupervisorLockContention.MixProject do
  use Mix.Project

  def project do
    [
      app: :partition_supervisor_lock_contention,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule MetricsAggregator.PartitionedAggregator do
  ...
  def dump do
    ...
    |> Enum.flat_map(fn p ->
      ...
      name |> Aggregator.dump() |> Map.to_list()
    end)
    |> Map.new()
  end
...
defmodule MetricsAggregator.PartitionedAggregatorTest do
  ...
  doctest MetricsAggregator.Application
...
## Why PartitionSupervisor to Reduce Lock Contention on Hot GenServers matters

Mastering **PartitionSupervisor to Reduce Lock Contention on Hot GenServers** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `lib/metrics_aggregator.ex`

```elixir
defmodule MetricsAggregator do
  @moduledoc """
  Reference implementation for PartitionSupervisor to Reduce Lock Contention on Hot GenServers.
  ...
  @doc """
  Entry point for the metrics_aggregator module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> MetricsAggregator.run(:noop)
      :ok
...
```
### `test/metrics_aggregator_test.exs`

```elixir
defmodule MetricsAggregatorTest do
  use ExUnit.Case, async: true

  doctest MetricsAggregator

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert MetricsAggregator.run(:noop) == :ok
    end
  end
end
```
