...
### `mix.exs`
```elixir
defmodule GracefulShutdownDrain.MixProject do
  use Mix.Project

  def project do
    [
      app: :graceful_shutdown_drain,
      version: "0.1.0",
      elixir: "~> 1.19",
...
```elixir
def process_request(req) do
  if accepting?() do
    GenServer.call(Server, {:process_request, req})
  else
    {:error, :draining}
  end
end
...
defmodule DrainShutdown.Server do
  ...
  @spec process_request(term()) :: {:ok, term()} | {:error, :draining}
  def process_request(req) do
    if accepting?() do
      GenServer.call(__MODULE__, {:process_request, req}, 30_000)
    else
      {:error, :draining}
    end
  end
  ...
  @impl true
  def handle_call({:process_request, req}, from, state) do
...
## Why Graceful shutdown and drain matters

Mastering **Graceful shutdown and drain** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `lib/drain_shutdown.ex`

```elixir
defmodule DrainShutdown do
  @moduledoc """
  Reference implementation for Graceful shutdown and drain.
  ...
  @doc """
  Entry point for the drain_shutdown module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> DrainShutdown.run(:noop)
      :ok
...
```
### `test/drain_shutdown_test.exs`

```elixir
defmodule DrainShutdownTest do
  use ExUnit.Case, async: true

  doctest DrainShutdown

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert DrainShutdown.run(:noop) == :ok
    end
  end
end
```
