...
## Why adopting orphan processes into a supervisor matters
...
### `lib/adoption_patterns.ex`

```elixir
defmodule AdoptionPatterns do
  @moduledoc """
  Adopting Orphan Processes into a Supervisor.
...
### `lib/adoption_patterns/application.ex`
...
### `lib/adoption_patterns/nursery.ex`
...
### `lib/adoption_patterns/adopted_worker.ex`
...
### `lib/adoption_patterns/legacy.ex`
...
defmodule AdoptionPatterns.NurseryTest do
  ...
  doctest AdoptionPatterns.Legacy
...
### `test/adoption_patterns_test.exs`

```elixir
defmodule AdoptionPatternsTest do
  use ExUnit.Case, async: true

  doctest AdoptionPatterns

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert AdoptionPatterns.run(:noop) == :ok
    end
  end
end
```
