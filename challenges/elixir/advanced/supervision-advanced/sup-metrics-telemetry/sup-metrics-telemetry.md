...
## Why supervisor metrics via `:telemetry` matters
...
```
### `lib/sup_metrics/application.ex`
...
defmodule SupMetrics.Reporter do
  ...
  defp snapshot(name) do
    ...
    e in RuntimeError -> Logger.warning("sup_metrics: snapshot failed for #{inspect(name)}: #{inspect(e)}")
  end
end
```
### `lib/sup_metrics/instrumentation.ex`
...
defmodule SupMetrics.SupervisorTest do
  ...
  doctest SupMetrics.Instrumentation
...
Main.main()
```
### `test/sup_metrics_test.exs`

```elixir
defmodule SupMetricsTest do
