...
## Why failure isolation between supervision subtrees matters
...
```
### `lib/failure_isolation.ex`

```elixir
defmodule FailureIsolation do
  @moduledoc """
  Failure Isolation Between Supervision Subtrees.
...
```
### `lib/failure_isolation/application.ex`
...
```
### `lib/failure_isolation/a/isolator.ex`
...
```
### `lib/failure_isolation/a/sup.ex`
...
```
### `lib/failure_isolation/a/worker.ex`
...
defmodule FailureIsolation.A.Worker do
  ...
  @spec process_request(term()) :: {:ok, term()} | no_return()
  def process_request(msg), do: GenServer.call(__MODULE__, {:process_request, msg})
  ...
  @impl true
  def handle_call({:process_request, msg}, _from, state) do
...
```
### `lib/failure_isolation/b/isolator.ex`
...
```
### `lib/failure_isolation/b/sup.ex`
...
```
### `lib/failure_isolation/b/worker.ex`
...
defmodule FailureIsolation.IsolationTest do
  ...
  doctest FailureIsolation.B.Worker
...
Main.main()
```
### `test/failure_isolation_test.exs`

```elixir
defmodule FailureIsolationTest do
  use ExUnit.Case, async: true

  doctest FailureIsolation

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert FailureIsolation.run(:noop) == :ok
    end
  end
end
```
