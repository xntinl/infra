...
### `mix.exs`
```elixir
defmodule PortDriverStreaming.MixProject do
  use Mix.Project

  def project do
    [
      app: :port_driver_streaming,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule MarketTape.DecoderPortTest do
  ...
  doctest MarketTape.Application
...
# script/main.exs
#
# Canonical entrypoint for the market_tape project. It wires the application
...
#
# The full implementation lives under `lib/market_tape/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `MarketTape` — ingest a high-volume binary market data feed from a native C decoder that runs continuously and streams parsed quotes into BEAM processes.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[market_tape] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[market_tape] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:market_tape) do
  ...
  defp run_demo do
    ...
    # For `market_tape`, the interesting entry points are documented above in
...
## Why Port Drivers for Long-Running External Processes matters

Mastering **Port Drivers for Long-Running External Processes** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
...
```elixir
defmodule MarketTape do
  @moduledoc """
  Reference implementation for Port Drivers for Long-Running External Processes.
  ...
  @doc """
  Entry point for the market_tape module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> MarketTape.run(:noop)
      :ok
...
### `test/market_tape_test.exs`

```elixir
defmodule MarketTapeTest do
