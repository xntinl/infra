...
### `lib/nif_resources.ex`

```elixir
defmodule NifResources do
  @moduledoc """
  NIF Resources — Managing Native Handles from Elixir.
...
defmodule NifResourcesTest do
  ...
  doctest NifResources.MixProject
...
# script/main.exs
#
# Canonical entrypoint for the nif_resources project. Run with:
#   mix deps.get && mix compile && mix run script/main.exs
# The Rust NIF under `native/nif_resources_nif/` is built by Rustler when
# `mix compile` runs for the first time.

defmodule Main do
  @moduledoc """
  Demo driver for `NifResources` — a NIF that exposes a mutable Rust HashMap
  to Elixir as an opaque resource handle with proper GC and drop semantics.
  """

  @spec main() :: :ok
  def main do
    IO.puts("[nif_resources] boot ok")

    case Application.ensure_all_started(:nif_resources) do
      {:ok, _} -> :ok
      {:error, _} -> :ok
    end

    IO.puts("[nif_resources] demo ok")
    :ok
  end
end
...
## Why NIF Resources — Managing Native Handles from Elixir matters

Mastering **NIF Resources — Managing Native Handles from Elixir** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
...
defmodule NifResourcesTest do
  ...
  doctest NifResources

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert NifResources.run(:noop) == :ok
    end
  end
end
```
