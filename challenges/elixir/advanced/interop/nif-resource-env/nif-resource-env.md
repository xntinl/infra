...
### `mix.exs`
```elixir
defmodule NifResourceEnv.MixProject do
  use Mix.Project

  def project do
    [
      app: :nif_resource_env,
      version: "0.1.0",
      elixir: "~> 1.19",
...
# script/main.exs
#
# Canonical entrypoint for the cache_lmdb project. It wires the application
...
#
# The full implementation lives under `lib/cache_lmdb/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `CacheLmdb` — embed LMDB (memory-mapped key-value store) into the BEAM, exposing opaque cursor and transaction handles as NIF resources with correct destructor semantics.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[cache_lmdb] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[cache_lmdb] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:cache_lmdb) do
  ...
  defp run_demo do
    ...
    # For `cache_lmdb`, the interesting entry points are documented above in
...
```elixir
defmodule CacheLmdb do
  @moduledoc """
  Reference implementation for NIF Resource Handles and the Env Lifetime.
  ...
  @doc """
  Entry point for the cache_lmdb module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> CacheLmdb.run(:noop)
      :ok
...
### `test/cache_lmdb_test.exs`

```elixir
defmodule CacheLmdbTest do
  use ExUnit.Case, async: true
