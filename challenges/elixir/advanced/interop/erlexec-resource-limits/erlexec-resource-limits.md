...
### `mix.exs`
```elixir
defmodule ErlexecResourceLimits.MixProject do
  use Mix.Project

  def project do
    [
      app: :erlexec_resource_limits,
      version: "0.1.0",
      elixir: "~> 1.19",
...
# script/main.exs
#
# Canonical entrypoint for the ml_sandbox project. It wires the application
...
#
# The full implementation lives under `lib/ml_sandbox/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `MlSandbox` — run user-submitted Python ML scripts inside resource-limited subprocesses with cgroups-style CPU and RSS caps.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[ml_sandbox] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[ml_sandbox] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:ml_sandbox) do
  ...
  defp run_demo do
    ...
    # For `ml_sandbox`, the interesting entry points are documented above in
...
## Why erlexec with CPU and Memory Limits matters

Mastering **erlexec with CPU and Memory Limits** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
...
```elixir
defmodule MlSandbox do
  @moduledoc """
  Reference implementation for erlexec with CPU and Memory Limits.
  ...
  @doc """
  Entry point for the ml_sandbox module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> MlSandbox.run(:noop)
      :ok
...
### `test/ml_sandbox_test.exs`

```elixir
defmodule MlSandboxTest do
