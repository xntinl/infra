...
### `mix.exs`
```elixir
defmodule SystemCmdTimeoutWrapper.MixProject do
  use Mix.Project

  def project do
    [
      app: :system_cmd_timeout_wrapper,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule MediaToolbox.CommandRunner do
  ...
  defp hard_kill(port, os_pid) do
    try do
      ...
      ArgumentError in RuntimeError -> :ok
    end
...
defmodule MediaToolbox.Exiftool do
  ...
  defp decode(bin) do
    try do
      ...
      e in RuntimeError -> {:error, :invalid_json}
    end
  end
end
```
...
defmodule MediaToolbox.CommandRunnerTest do
  ...
  doctest MediaToolbox.Application
  alias MediaToolbox.CommandRunner
...
## Why Safe `System.cmd` Wrappers for Native CLI Tools matters

Mastering **Safe `System.cmd` Wrappers for Native CLI Tools** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `lib/media_toolbox.ex`

```elixir
defmodule MediaToolbox do
  @moduledoc """
  Reference implementation for Safe `System.cmd` Wrappers for Native CLI Tools.
  ...
  @doc """
  Entry point for the media_toolbox module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> MediaToolbox.run(:noop)
      :ok
...
```
### `test/media_toolbox_test.exs`

```elixir
defmodule MediaToolboxTest do
  use ExUnit.Case, async: true

  doctest MediaToolbox

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert MediaToolbox.run(:noop) == :ok
    end
  end
end
```
