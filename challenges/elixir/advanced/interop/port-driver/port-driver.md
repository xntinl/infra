...
### `lib/port_driver_demo.ex`

```elixir
defmodule PortDriverDemo do
  @moduledoc """
  Port Drivers vs Ports — When C Meets BEAM Without a Pipe.
...
### `lib/port_driver_demo/via_port.ex`
...
### `lib/port_driver_demo/via_driver.ex`
...
defmodule PortDriverDemo.ViaDriver do
  ...
  def upcase(data) when is_binary(data) do
    ...
    result =
      receive do
        {^port, {:data, d}} -> d
      after
        1_000 -> raise ArgumentError, "driver timeout"
      end
  ...
  defp ensure_loaded do
    case :erl_ddll.load_driver(String.to_charlist(@priv_path), ~c"upcase_drv") do
      ...
      {:error, reason} -> raise ArgumentError, "driver load: #{inspect(reason)}"
    end
  end
end
```

### `lib/port_driver_demo/via_nif.ex`
...
defmodule PortDriverDemoTest do
  ...
  doctest PortDriverDemo.MixProject
...
## Why Port Drivers vs Ports — When C Meets BEAM Without a Pipe matters

Mastering **Port Drivers vs Ports — When C Meets BEAM Without a Pipe** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `test/port_driver_demo_test.exs`

```elixir
defmodule PortDriverDemoTest do
  use ExUnit.Case, async: true

  doctest PortDriverDemo

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert PortDriverDemo.run(:noop) == :ok
    end
  end
end
```
