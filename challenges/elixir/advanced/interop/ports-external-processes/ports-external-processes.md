...
```elixir
defmodule PortsExternalProcesses.MixProject do
  use Mix.Project

  def project do
    [
      app: :ports_external_processes,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule PortDemo.MixProject do
  ...
  def project, do: [app: :port_demo, version: "0.1.0", elixir: "~> 1.19", deps: []]
...
defmodule PortDemo.Application do
  @moduledoc """
  Ejercicio: Ports — Wrapping External Processes Safely.
...
# script/main.exs
#
# Canonical entrypoint for the port_demo project. It wires the application
...
#
# The full implementation lives under `lib/port_demo/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `PortDemo` — a long-lived wrapper around an external CLI (`ffmpeg`, `imagemagick`, a Python ML model) exposed as an OTP GenServer to the rest of the system.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[port_demo] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[port_demo] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:port_demo) do
  ...
  defp run_demo do
    ...
    # For `port_demo`, the interesting entry points are documented above in
...
## Why Ports — Wrapping External Processes Safely matters

Mastering **Ports — Wrapping External Processes Safely** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `test/port_demo_test.exs`
