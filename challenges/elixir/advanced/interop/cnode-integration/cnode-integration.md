...
### `mix.exs`
```elixir
defmodule CnodeIntegration.MixProject do
  use Mix.Project

  def project do
    [
      app: :cnode_integration,
      version: "0.1.0",
      elixir: "~> 1.19",
...
# script/main.exs
#
# Canonical entrypoint for the hw_bridge project. It wires the application
...
#
# The full implementation lives under `lib/hw_bridge/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `HwBridge` — a C program that talks to a specialized hardware SDK appears as an Erlang node on the cluster, receives RPC calls from Elixir, and returns telemetry.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[hw_bridge] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[hw_bridge] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:hw_bridge) do
  ...
  defp run_demo do
    ...
    # For `hw_bridge`, the interesting entry points are documented above in
...
## Why C Node Integration over Erlang Distribution matters

Mastering **C Node Integration over Erlang Distribution** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
...
```elixir
defmodule HwBridge do
  @moduledoc """
  Reference implementation for C Node Integration over Erlang Distribution.
  ...
  @doc """
  Entry point for the hw_bridge module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> HwBridge.run(:noop)
      :ok
...
### `test/hw_bridge_test.exs`

```elixir
defmodule HwBridgeTest do
