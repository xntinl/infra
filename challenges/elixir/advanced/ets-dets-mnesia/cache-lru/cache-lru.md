...
```elixir
defmodule CacheLru.MixProject do
  use Mix.Project
...
defmodule LruCache.MixProject do
  ...
  def project do
    [app: :lru_cache, version: "0.1.0", elixir: "~> 1.19", deps: deps()]
  end
...
defmodule LruCache do
  ...
  A GenServer serializes all reads through one process. An ETS-backed LRU lets readers touch the table directly and only serializes the eviction step, which runs on a timer or at....
  """
end
```

### `lib/lru_cache/application.ex`
...
# script/main.exs
#
# Canonical entrypoint for the lru_cache project. It wires the application
...
#
# The full implementation lives under `lib/lru_cache/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `LruCache` — bounded LRU cache with O(1) eviction.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[lru_cache] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[lru_cache] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:lru_cache) do
  ...
  defp run_demo do
    ...
    # For `lru_cache`, the interesting entry points are documented above in
...
## Why LRU Cache on ETS with a Doubly-Linked List matters

Mastering **LRU Cache on ETS with a Doubly-Linked List** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
