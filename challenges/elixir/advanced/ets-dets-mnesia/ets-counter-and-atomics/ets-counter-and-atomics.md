...
```elixir
defmodule EtsCounterAndAtomics.MixProject do
  use Mix.Project

  def project do
    [
      app: :ets_counter_and_atomics,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule EtsAtomics.MixProject do
  ...
  def project do
    [app: :ets_atomics, version: "0.1.0", elixir: "~> 1.19", deps: deps()]
  end
...
```elixir
defmodule EtsAtomics do
  @moduledoc """
  Counters: `:ets.update_counter` vs `:counters` vs `:atomics`.

  A counter that goes through a GenServer is an integer with a message queue. An `:atomics` array is a single hardware instruction per update. The difference is orders of magnitude....
  """
end
```

### `lib/ets_atomics/application.ex`
...
### `lib/ets_atomics/ets_counter.ex`
...
### `lib/ets_atomics/counters_counter.ex`
...
### `lib/ets_atomics/atomics_counter.ex`
...
defmodule EtsAtomicsTest do
  ...
  doctest EtsAtomics.AtomicsCounter
...
# script/main.exs
#
# Canonical entrypoint for the ets_atomics project. It wires the application
...
#
# The full implementation lives under `lib/ets_atomics/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `EtsAtomics` — a micro-benchmark suite that contrasts three lock-free counter mechanisms and maps each to the production scenarios where it's the right choice.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[ets_atomics] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[ets_atomics] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:ets_atomics) do
  ...
  defp run_demo do
    ...
    # For `ets_atomics`, the interesting entry points are documented above in
...
## Why Counters matters

Mastering **Counters** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `test/ets_atomics_test.exs`
