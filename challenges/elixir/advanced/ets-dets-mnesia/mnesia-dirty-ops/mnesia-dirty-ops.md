...
```elixir
defmodule MnesiaDirtyOps.MixProject do
  use Mix.Project

  def project do
    [
      app: :mnesia_dirty_ops,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule MnesiaDirty.MixProject do
  ...
  def project do
    [app: :mnesia_dirty, version: "0.1.0", elixir: "~> 1.19", deps: deps()]
  end
...
```elixir
defmodule MnesiaDirty do
  @moduledoc """
  Mnesia Dirty Operations — When to Skip Transactions.

  A Mnesia transaction involves the transaction manager, lock acquisition, and replication coordination. A dirty read bypasses all of that and reads directly from the local ETS....
  """
end
```

### `lib/mnesia_dirty/application.ex`
...
### `lib/mnesia_dirty/metrics_reporter.ex`
...
defmodule MnesiaDirty.CountersTest do
  ...
  doctest MnesiaDirty.MetricsReporter
...
# script/main.exs
#
# Canonical entrypoint for the mnesia_dirty project. It wires the application
...
#
# The full implementation lives under `lib/mnesia_dirty/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `MnesiaDirty` — high-throughput counters and read-heavy lookup tables.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[mnesia_dirty] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[mnesia_dirty] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:mnesia_dirty) do
  ...
  defp run_demo do
    ...
    # For `mnesia_dirty`, the interesting entry points are documented above in
...
defmodule MnesiaDirtyTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert MnesiaDirty.run(:noop) == :ok
    end
  end
end
```
