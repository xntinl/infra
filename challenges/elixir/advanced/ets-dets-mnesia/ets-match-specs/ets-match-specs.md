...
defmodule SessionStore.MixProject do
  ...
  def project do
    [app: :session_store, version: "0.1.0", elixir: "~> 1.19", deps: deps()]
  end
...
## Why ETS Match Specs with ` matters

Mastering **ETS Match Specs with `** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `lib/session_store.ex`

```elixir
defmodule SessionStore do
  @moduledoc """
  Reference implementation for ETS Match Specs with `:ets.fun2ms` and `:ets.select`.
  ...
  @doc """
  Entry point for the session_store module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> SessionStore.run(:noop)
      :ok
...
### `test/session_store_test.exs`

```elixir
defmodule SessionStoreTest do
  use ExUnit.Case, async: true

  doctest SessionStore

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert SessionStore.run(:noop) == :ok
    end
  end
end
```
