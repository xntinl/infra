...
## Why deep dive into `:trap_exit` semantics matters
...
```elixir
defmodule TrapExitSemantics.MixProject do
  use Mix.Project

  def project do
    [
      app: :trap_exit_semantics,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule TrapExitDeep.MixProject do
  use Mix.Project
  def project, do: [app: :trap_exit_deep, version: "0.1.0", elixir: "~> 1.19", deps: []]
  def application, do: [extra_applications: [:logger]]
end
```

### `lib/trap_exit_deep.ex`

```elixir
defmodule TrapExitDeep do
  @moduledoc """
  Deep Dive into `:trap_exit` Semantics.
...
### `lib/trap_exit_deep/signal_matrix.ex`
...
### `lib/trap_exit_deep/trapping_worker.ex`
...
### `lib/trap_exit_deep/application.ex`
...
defmodule TrapExitDeep.SignalMatrixTest do
  ...
  doctest TrapExitDeep.Application
...
# script/main.exs
#
# Canonical entrypoint for the trap_exit_deep project. It wires the application
...
#
# The full implementation lives under `lib/trap_exit_deep/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `TrapExitDeep` — mastering exit signal propagation, `:kill` vs `:killed`, and supervisor interactions.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[trap_exit_deep] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[trap_exit_deep] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:trap_exit_deep) do
  ...
  defp run_demo do
    ...
    # For `trap_exit_deep`, the interesting entry points are documented above in
...
### `test/trap_exit_deep_test.exs`

```elixir
defmodule TrapExitDeepTest do
  use ExUnit.Case, async: true

  doctest TrapExitDeep

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert TrapExitDeep.run(:noop) == :ok
    end
  end
end
```
