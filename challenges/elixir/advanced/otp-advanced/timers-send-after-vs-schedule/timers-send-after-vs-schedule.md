...
## Why timers on the beam: `send_after` vs `:timer` vs `start_timer` matters
...
### `lib/timer_comparison.ex`

```elixir
defmodule TimerComparison do
  @moduledoc """
  Timers on the BEAM: `send_after` vs `:timer` vs `start_timer`.
...
### `lib/timer_comparison/application.ex`
...
### `lib/timer_comparison/send_after_worker.ex`
...
### `lib/timer_comparison/stdlib_timer_worker.ex`
...
### `lib/timer_comparison/start_timer_worker.ex`
...
defmodule TimerComparisonTest do
  ...
  doctest TimerComparison.StartTimerWorker
...
### `test/timer_comparison_test.exs`

```elixir
defmodule TimerComparisonTest do
  use ExUnit.Case, async: true

  doctest TimerComparison

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert TimerComparison.run(:noop) == :ok
    end
  end
end
```
