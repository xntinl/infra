...
### `mix.exs`
```elixir
defmodule HotCodeUpgradesReleaseHandler.MixProject do
  use Mix.Project

  def project do
    [
      app: :hot_code_upgrades_release_handler,
      version: "0.1.0",
      elixir: "~> 1.19",
...
# script/main.exs
#
# Canonical entrypoint for the coin_counter project. It wires the application
...
#
# The full implementation lives under `lib/coin_counter/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `CoinCounter` — GenServer running as a release, upgraded from v1 (counts clicks) to v2 (counts clicks and records timestamps) without restarting the node, using `relup` scripts and the `release_handler`.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[coin_counter] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[coin_counter] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:coin_counter) do
  ...
  defp run_demo do
    ...
    # For `coin_counter`, the interesting entry points are documented above in
...
```elixir
defmodule CoinCounter do
  @moduledoc """
  Reference implementation for Hot Code Upgrades with Release Handlers.
  ...
  @doc """
  Entry point for the coin_counter module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> CoinCounter.run(:noop)
      :ok
...
```
### `test/coin_counter_test.exs`

```elixir
defmodule CoinCounterTest do
  use ExUnit.Case, async: true
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert CoinCounter.run(:noop) == :ok
    end
  end
end
```
