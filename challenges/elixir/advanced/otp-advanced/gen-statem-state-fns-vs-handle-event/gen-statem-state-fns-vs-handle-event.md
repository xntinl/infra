...
### `mix.exs`
```elixir
defmodule GenStatemStateFunctionsVsHandleEvent.MixProject do
  use Mix.Project

  def project do
    [
      app: :gen_statem_state_functions_vs_handle_event,
      version: "0.1.0",
      elixir: "~> 1.19",
...
# script/main.exs
#
# Canonical entrypoint for the door_controller project. It wires the application
...
#
# The full implementation lives under `lib/door_controller/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `DoorController` — smart-lock state machine implemented twice, once with `:state_functions` callback mode and once with `:handle_event_function`, to compare ergonomics, code locality, and the cost of each style.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[door_controller] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[door_controller] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:door_controller) do
  ...
  defp run_demo do
    ...
    # For `door_controller`, the interesting entry points are documented above in
...
## Why `:gen_statem` — State Functions vs `handle_event_function` matters

Mastering **`:gen_statem` — State Functions vs `handle_event_function`** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `lib/door_controller.ex`

```elixir
defmodule DoorController do
  @moduledoc """
  Reference implementation for `:gen_statem` — State Functions vs `handle_event_function`.
  ...
  @doc """
  Entry point for the door_controller module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> DoorController.run(:noop)
      :ok
...
### `test/door_controller_test.exs`

```elixir
defmodule DoorControllerTest do
  use ExUnit.Case, async: true

  doctest DoorController

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert DoorController.run(:noop) == :ok
    end
  end
end
```
