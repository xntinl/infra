...
defmodule ContinueRecoveryGs.MixProject do
  ...
  def project do
    [app: :continue_recovery_gs, version: "0.1.0", elixir: "~> 1.19", deps: []]
  end
...
### `lib/continue_recovery_gs.ex`

```elixir
defmodule ContinueRecoveryGs do
  @moduledoc """
  GenServer `handle_continue/2` and State Recovery.

  You could spawn a task from `init/1` to load DETS and `send` results back. That returns control quickly too, but it drops OTP's ordering guarantee: between the moment `init`....
  """
end
```

### `lib/continue_recovery_gs/dets_store.ex`
...
### `lib/continue_recovery_gs/offsets.ex`
...
### `lib/continue_recovery_gs/application.ex`
...
defmodule ContinueRecoveryGs.RecoveryTest do
  ...
  doctest ContinueRecoveryGs.Application

  alias ContinueRecoveryGs.Offsets

  setup do
    path = "priv/dets/test_#{System.unique_integer([:positive])}.dets"
    store = String.to_existing_atom("store_#{System.unique_integer([:positive])}")
...
# script/main.exs
#
# Canonical entrypoint for the continue_recovery_gs project. It wires the application
...
#
# The full implementation lives under `lib/continue_recovery_gs/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `ContinueRecoveryGs` — a crash-recovering GenServer that survives restarts via DETS and never blocks its supervisor at boot.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[continue_recovery_gs] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[continue_recovery_gs] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:continue_recovery_gs) do
  ...
  defp run_demo do
    ...
    # For `continue_recovery_gs`, the interesting entry points are documented above in
...
## Why GenServer `handle_continue/2` and State Recovery matters

Mastering **GenServer `handle_continue/2` and State Recovery** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `test/continue_recovery_gs_test.exs`

```elixir
defmodule ContinueRecoveryGsTest do
  use ExUnit.Case, async: true

  doctest ContinueRecoveryGs

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert ContinueRecoveryGs.run(:noop) == :ok
    end
  end
end
```
