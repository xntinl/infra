...
## Why implementing an otp special process from scratch matters
...
### `lib/special_process/counter.ex`
...
### `lib/special_process/counter_supervisor.ex`
...
### `lib/special_process/application.ex`
...
defmodule SpecialProcess.CounterTest do
  ...
  doctest SpecialProcess.Application
...
### `test/special_process_test.exs`

```elixir
defmodule SpecialProcessTest do
  use ExUnit.Case, async: true

  doctest SpecialProcess

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert SpecialProcess.run(:noop) == :ok
    end
  end
end
```
