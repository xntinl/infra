...
### `mix.exs`
```elixir
defmodule ProcLibSysHandrolledBehaviour.MixProject do
  use Mix.Project

  def project do
    [
      app: :proc_lib_sys_handrolled_behaviour,
      version: "0.1.0",
      elixir: "~> 1.19",
...
# script/main.exs
#
# Canonical entrypoint for the tiny_server project. It wires the application
...
#
# The full implementation lives under `lib/tiny_server/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `TinyServer` — a special process implemented from scratch with `:proc_lib`, `:sys`, and a custom receive loop, so you can handle `:sys` debug commands, system messages, and supervision-tree hand-off without wrapping `GenServer`.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[tiny_server] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[tiny_server] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:tiny_server) do
  ...
  defp run_demo do
    ...
    # For `tiny_server`, the interesting entry points are documented above in
...
## Why Hand-Rolled Behaviour with ` matters

Mastering **Hand-Rolled Behaviour with `** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
...
defmodule TinyServer do
  @moduledoc """
  Reference implementation for Hand-Rolled Behaviour with `:proc_lib` and `:sys`.
  ...
  @doc """
  Entry point for the tiny_server module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> TinyServer.run(:noop)
      :ok
