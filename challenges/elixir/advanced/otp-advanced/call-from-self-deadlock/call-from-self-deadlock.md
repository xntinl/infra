...
## Why `genserver.call` to self: the canonical deadlock matters
...
defmodule DeadlockDemo.MixProject do
  use Mix.Project
  def project, do: [app: :deadlock_demo, version: "0.1.0", elixir: "~> 1.19", deps: []]
...
### `lib/deadlock_demo/deadlocking.ex`
...
### `lib/deadlock_demo/with_continue.ex`
...
### `lib/deadlock_demo/with_info.ex`
...
### `lib/deadlock_demo/with_cast.ex`
...
### `lib/deadlock_demo/split_module.ex`
...
defmodule DeadlockDemoTest do
  ...
  doctest DeadlockDemo.OrderLogic
...
### `test/deadlock_demo_test.exs`

```elixir
defmodule DeadlockDemoTest do
  use ExUnit.Case, async: true

  doctest DeadlockDemo

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert DeadlockDemo.run(:noop) == :ok
    end
  end
end
```
