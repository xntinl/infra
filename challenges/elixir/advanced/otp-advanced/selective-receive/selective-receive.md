...
## Why selective receive and the mailbox scan trap matters
...
defmodule SelectiveReceiveDemo.MixProject do
  ...
  def project, do: [app: :selective_receive_demo, version: "0.1.0", elixir: "~> 1.19", deps: deps()]
...
```
### `lib/selective_receive_demo.ex`

```elixir
defmodule SelectiveReceiveDemo do
  @moduledoc """
  Selective Receive and the Mailbox Scan Trap.
...
```
### `lib/selective_receive_demo/naive_receiver.ex`
...
defmodule SelectiveReceiveDemo.NaiveReceiver do
  ...
  def handle_high(pid, iterations) do
    ...
    after
      120_000 -> raise ArgumentError, "timeout"
    end
  end
  ...
  defp drain_high(n) do
    ...
    after
      10_000 -> raise ArgumentError, "missing high-priority message"
    end
  end
end
```
### `lib/selective_receive_demo/fixed_receiver.ex`
...
```
### `lib/selective_receive_demo/application.ex`
...
defmodule SelectiveReceiveDemo.ReceiverTest do
  ...
  doctest SelectiveReceiveDemo.Application
...
Main.main()
```
### `test/selective_receive_demo_test.exs`

```elixir
defmodule SelectiveReceiveDemoTest do
  use ExUnit.Case, async: true

  doctest SelectiveReceiveDemo

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert SelectiveReceiveDemo.run(:noop) == :ok
    end
  end
end
```
...
defp loop(state) do
  receive do
    msg ->
      state = process_request(msg, state)
      loop(state)
  end
end

defp process_request({:high_priority, x}, state), do: ...
defp process_request({:low_priority, x}, state), do: ...
defp process_request(_unknown, state), do: state
```
