...
## Why `:gen_statem` retry with exponential backoff matters
...
defmodule RetryFsm.MixProject do
  ...
  def project, do: [app: :retry_fsm, version: "0.1.0", elixir: "~> 1.19", deps: deps()]
...
defmodule RetryFsm.Worker do
  ...
  defp safely_run(fun) do
    try do
      ...
      e in RuntimeError -> {:error, {:exception, Exception.message(e)}}
    end
  end
...
Main.main()
```
### `test/retry_fsm_test.exs`

```elixir
defmodule RetryFsmTest do
  use ExUnit.Case, async: true
