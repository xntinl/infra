...
## Why `:proc_lib.start_link` vs `spawn_link`: the otp handshake matters
...
```elixir
defmodule ProcLibStartLink.MixProject do
  use Mix.Project

  def project do
    [
      app: :proc_lib_start_link,
      version: "0.1.0",
      elixir: "~> 1.19",
...
### `lib/proc_lib_worker.ex`

```elixir
defmodule ProcLibWorker do
  @moduledoc """
  `:proc_lib.start_link` vs `spawn_link`: the OTP handshake.
...
### `lib/proc_lib_worker/naive_worker.ex`
...
### `lib/proc_lib_worker/ok_worker.ex`
...
### `lib/proc_lib_worker/application.ex`
...
defmodule ProcLibWorker.NaiveWorkerTest do
  ...
  doctest ProcLibWorker.Application
...
# script/main.exs
#
# Canonical entrypoint for the proc_lib_worker project. It wires the application
...
#
# The full implementation lives under `lib/proc_lib_worker/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `ProcLibWorker` — understanding the `init_ack` protocol that separates OTP-compliant processes from raw spawns.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[proc_lib_worker] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[proc_lib_worker] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:proc_lib_worker) do
  ...
  defp run_demo do
    ...
    # For `proc_lib_worker`, the interesting entry points are documented above in
...
### `test/proc_lib_worker_test.exs`

```elixir
defmodule ProcLibWorkerTest do
  use ExUnit.Case, async: true

  doctest ProcLibWorker

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert ProcLibWorker.run(:noop) == :ok
    end
  end
end
```
