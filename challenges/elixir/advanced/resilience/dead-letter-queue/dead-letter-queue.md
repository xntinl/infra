...
### `mix.exs`
```elixir
defmodule DeadLetterQueue.MixProject do
  use Mix.Project

  def project do
    [
      app: :dead_letter_queue,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule WebhookDlq.MixProject do
  use Mix.Project
  def project, do: [app: :webhook_dlq, version: "0.1.0", elixir: "~> 1.19", deps: []]
...
# script/main.exs
#
# Canonical entrypoint for the webhook_dlq project. It wires the application
...
#
# The full implementation lives under `lib/webhook_dlq/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `WebhookDlq` — a webhook delivery system that retries failed deliveries with backoff and, after exhausting attempts, parks undeliverable messages in a Dead Letter Queue for human inspection.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[webhook_dlq] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[webhook_dlq] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:webhook_dlq) do
  ...
  defp run_demo do
    ...
    # For `webhook_dlq`, the interesting entry points are documented above in
...
## Why Dead Letter Queue Patterns matters

Mastering **Dead Letter Queue Patterns** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
...
```elixir
defmodule WebhookDlq do
  @moduledoc """
  Ejercicio: Dead Letter Queue Patterns.
  Implementa el comportamiento descrito en el enunciado, exponiendo funciones públicas documentadas y con tipos claros.
  """

  @doc """
  Entry point for the webhook_dlq module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> WebhookDlq.run(:noop)
      :ok
...
### `test/webhook_dlq_test.exs`

```elixir
defmodule WebhookDlqTest do
