...
### `mix.exs`
```elixir
defmodule CircuitBreakerPatterns.MixProject do
  use Mix.Project

  def project do
    [
      app: :circuit_breaker_patterns,
      version: "0.1.0",
      elixir: "~> 1.19",
...
### `lib/circuit_breaker_patterns.ex`

```elixir
defmodule CircuitBreakerPatterns do
  @moduledoc """
  Circuit Breaker Patterns — Manual State Machine.
...
### `lib/circuit_breaker_patterns/classifier.ex`
...
### `lib/circuit_breaker_patterns/breaker.ex`
...
# script/main.exs
#
# Canonical entrypoint for the circuit_breaker_patterns project. It wires the application
...
#
# The full implementation lives under `lib/circuit_breaker_patterns/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `CircuitBreakerPatterns` — a hand-rolled circuit breaker with closed / open / half-open states, telemetry, and production-grade failure classification.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[circuit_breaker_patterns] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[circuit_breaker_patterns] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:circuit_breaker_patterns) do
  ...
  defp run_demo do
    ...
    # For `circuit_breaker_patterns`, the interesting entry points are documented above in
...
## Why Circuit Breaker Patterns — Manual State Machine matters

Mastering **Circuit Breaker Patterns — Manual State Machine** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `test/circuit_breaker_patterns_test.exs`

```elixir
defmodule CircuitBreakerPatternsTest do
  use ExUnit.Case, async: true

  doctest CircuitBreakerPatterns

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert CircuitBreakerPatterns.run(:noop) == :ok
    end
  end
end
```
