...
### `mix.exs`
```elixir
defmodule BoundedQueueGenstageBackpressure.MixProject do
  use Mix.Project

  def project do
    [
      app: :bounded_queue_genstage_backpressure,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule IngestPipeline.MixProject do
  ...
  def project do
    [app: :ingest_pipeline, version: "0.1.0", elixir: "~> 1.19", deps: deps()]
  end
...
# script/main.exs
#
# Canonical entrypoint for the ingest_pipeline project. It wires the application
...
#
# The full implementation lives under `lib/ingest_pipeline/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `IngestPipeline` — a producer-consumer pipeline where a Kafka-like producer only emits events when consumers demand capacity, guaranteeing bounded memory.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[ingest_pipeline] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[ingest_pipeline] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:ingest_pipeline) do
  ...
  defp run_demo do
    ...
    # For `ingest_pipeline`, the interesting entry points are documented above in
...
## Why Bounded Queue with GenStage Back-Pressure matters

Mastering **Bounded Queue with GenStage Back-Pressure** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
...
```elixir
defmodule IngestPipeline do
  @moduledoc """
  Ejercicio: Bounded Queue with GenStage Back-Pressure.
  Implementa el comportamiento descrito en el enunciado, exponiendo funciones públicas documentadas y con tipos claros.
  """

  @doc """
  Entry point for the ingest_pipeline module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> IngestPipeline.run(:noop)
      :ok
...
```elixir
defmodule IngestPipelineTest do
  use ExUnit.Case, async: true

  doctest IngestPipeline

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert IngestPipeline.run(:noop) == :ok
    end
  end
end
```
