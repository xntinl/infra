...
### `mix.exs`
```elixir
defmodule SagaCompensatingActions.MixProject do
  use Mix.Project

  def project do
    [
      app: :saga_compensating_actions,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule BookingSaga.MixProject do
  use Mix.Project
  def project, do: [app: :booking_saga, version: "0.1.0", elixir: "~> 1.19", deps: []]
...
## Why Saga Pattern with Compensating Actions matters

Mastering **Saga Pattern with Compensating Actions** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
...
```elixir
defmodule BookingSaga do
  @moduledoc """
  Ejercicio: Saga Pattern with Compensating Actions.
  Implementa el comportamiento descrito en el enunciado, exponiendo funciones públicas documentadas y con tipos claros.
  """

  @doc """
  Entry point for the booking_saga module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> BookingSaga.run(:noop)
      :ok
...
```
### `test/booking_saga_test.exs`

```elixir
defmodule BookingSagaTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert BookingSaga.run(:noop) == :ok
    end
  end
end
```
