...
### `mix.exs`
```elixir
defmodule AdaptiveConcurrencyLimits.MixProject do
  use Mix.Project

  def project do
    [
      app: :adaptive_concurrency_limits,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule AdaptiveLimiter.MixProject do
  use Mix.Project
  def project, do: [app: :adaptive_limiter, version: "0.1.0", elixir: "~> 1.19", deps: []]
...
defmodule AdaptiveLimiter.GradientTest do
  ...
  doctest AdaptiveLimiter.Limiter
  alias AdaptiveLimiter.Gradient
...
# script/main.exs
#
# Canonical entrypoint for the adaptive_limiter project. It wires the application
...
#
# The full implementation lives under `lib/adaptive_limiter/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `AdaptiveLimiter` — dynamically adjusts the concurrency limit protecting a downstream service based on observed latency, inspired by Netflix's `concurrency-limits` library and TCP congestion control.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[adaptive_limiter] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[adaptive_limiter] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:adaptive_limiter) do
  ...
  defp run_demo do
    ...
    # For `adaptive_limiter`, the interesting entry points are documented above in
...
## Why Adaptive Concurrency Limits matters

Mastering **Adaptive Concurrency Limits** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `lib/adaptive_limiter.ex`

```elixir
defmodule AdaptiveLimiter do
  @moduledoc """
  Reference implementation for Adaptive Concurrency Limits.
  ...
  @doc """
  Entry point for the adaptive_limiter module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> AdaptiveLimiter.run(:noop)
      :ok
...
### `test/adaptive_limiter_test.exs`

```elixir
defmodule AdaptiveLimiterTest do
  use ExUnit.Case, async: true

  doctest AdaptiveLimiter

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert AdaptiveLimiter.run(:noop) == :ok
    end
  end
end
```
