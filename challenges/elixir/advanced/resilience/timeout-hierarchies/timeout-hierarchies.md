...
defmodule DeadlineRpc.MixProject do
  use Mix.Project
  def project, do: [app: :deadline_rpc, version: "0.1.0", elixir: "~> 1.19", deps: []]
...
## Why Timeout Hierarchies and Deadline Propagation matters

Mastering **Timeout Hierarchies and Deadline Propagation** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
...
```elixir
defmodule DeadlineRpc do
  @moduledoc """
  Reference implementation for Timeout Hierarchies and Deadline Propagation.
  ...
  @doc """
  Entry point for the deadline_rpc module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> DeadlineRpc.run(:noop)
      :ok
...
### `test/deadline_rpc_test.exs`

```elixir
defmodule DeadlineRpcTest do
  use ExUnit.Case, async: true
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert DeadlineRpc.run(:noop) == :ok
    end
  end
end
```
...
### `mix.exs`
```elixir
defmodule TimeoutHierarchies.MixProject do
  use Mix.Project

  def project do
    [
      app: :timeout_hierarchies,
      version: "0.1.0",
      elixir: "~> 1.19",
