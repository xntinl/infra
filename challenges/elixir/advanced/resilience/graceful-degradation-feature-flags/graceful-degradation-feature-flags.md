...
defmodule CheckoutDegrader.MixProject do
  use Mix.Project
  def project, do: [app: :checkout_degrader, version: "0.1.0", elixir: "~> 1.19", deps: []]
...
defmodule CheckoutDegrader.FlagsTest do
  ...
  doctest CheckoutDegrader.Checkout
...
## Why Graceful Degradation with Feature Flags matters

Mastering **Graceful Degradation with Feature Flags** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `lib/checkout_degrader.ex`

```elixir
defmodule CheckoutDegrader do
  @moduledoc """
  Ejercicio: Graceful Degradation with Feature Flags.
  Implementa el comportamiento descrito en el enunciado, exponiendo funciones públicas documentadas y con tipos claros.
  """

  @doc """
  Entry point for the checkout_degrader module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> CheckoutDegrader.run(:noop)
      :ok
...
### `test/checkout_degrader_test.exs`

```elixir
defmodule CheckoutDegraderTest do
  use ExUnit.Case, async: true

  doctest CheckoutDegrader

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert CheckoutDegrader.run(:noop) == :ok
    end
  end
end
```
...
### `mix.exs`
```elixir
defmodule GracefulDegradationFeatureFlags.MixProject do
  use Mix.Project

  def project do
    [
      app: :graceful_degradation_feature_flags,
      version: "0.1.0",
      elixir: "~> 1.19",
