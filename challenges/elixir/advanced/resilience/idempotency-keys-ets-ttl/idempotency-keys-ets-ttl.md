...
### `mix.exs`
```elixir
defmodule IdempotencyKeysEtsTtl.MixProject do
  use Mix.Project

  def project do
    [
      app: :idempotency_keys_ets_ttl,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule ChargeIdempotency.MixProject do
  use Mix.Project
  def project, do: [app: :charge_idempotency, version: "0.1.0", elixir: "~> 1.19", deps: deps()]
...
defmodule ChargeIdempotency.StoreTest do
  ...
  doctest ChargeIdempotency.Service
  alias ChargeIdempotency.Store
...
# script/main.exs
#
# Canonical entrypoint for the charge_idempotency project. It wires the application
...
#
# The full implementation lives under `lib/charge_idempotency/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `ChargeIdempotency` — an idempotency layer for a payments API that dedups retried requests within a TTL window using ETS and a lightweight sweeper.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[charge_idempotency] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[charge_idempotency] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:charge_idempotency) do
  ...
  defp run_demo do
    ...
    # For `charge_idempotency`, the interesting entry points are documented above in
...
## Why Idempotency Keys with ETS and TTL matters

Mastering **Idempotency Keys with ETS and TTL** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `lib/charge_idempotency.ex`

```elixir
defmodule ChargeIdempotency do
  @moduledoc """
  Ejercicio: Idempotency Keys with ETS and TTL.
  Implementa el comportamiento descrito en el enunciado, exponiendo funciones públicas documentadas y con tipos claros.
  """

  @doc """
  Entry point for the charge_idempotency module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> ChargeIdempotency.run(:noop)
      :ok
...
```
### `test/charge_idempotency_test.exs`

```elixir
defmodule ChargeIdempotencyTest do
  use ExUnit.Case, async: true

  doctest ChargeIdempotency

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert ChargeIdempotency.run(:noop) == :ok
    end
  end
end
```
