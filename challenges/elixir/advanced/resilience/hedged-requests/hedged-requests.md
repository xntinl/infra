...
### `mix.exs`
```elixir
defmodule HedgedRequests.MixProject do
  use Mix.Project
...
defmodule SearchHedger.MixProject do
  use Mix.Project
  def project, do: [app: :search_hedger, version: "0.1.0", elixir: "~> 1.19", deps: deps()]
...
# script/main.exs
#
# Canonical entrypoint for the search_hedger project. It wires the application
...
#
# The full implementation lives under `lib/search_hedger/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `SearchHedger` — reduces tail latency on search queries by firing a second duplicate request after a delay; first successful response wins and the loser is cancelled.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[search_hedger] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[search_hedger] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:search_hedger) do
  ...
  defp run_demo do
    ...
    # For `search_hedger`, the interesting entry points are documented above in
...
## Why Hedged Requests matters

Mastering **Hedged Requests** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `lib/search_hedger.ex`

```elixir
defmodule SearchHedger do
  @moduledoc """
  Ejercicio: Hedged Requests.
  Implementa el comportamiento descrito en el enunciado, exponiendo funciones públicas documentadas y con tipos claros.
  """

  @doc """
  Entry point for the search_hedger module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> SearchHedger.run(:noop)
      :ok
...
### `test/search_hedger_test.exs`

```elixir
defmodule SearchHedgerTest do
  use ExUnit.Case, async: true

  doctest SearchHedger

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert SearchHedger.run(:noop) == :ok
    end
  end
end
```
