...
### `mix.exs`
```elixir
defmodule RetryExponentialBackoffJitter.MixProject do
  use Mix.Project

  def project do
    [
      app: :retry_exponential_backoff_jitter,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule ResilientHttp.MixProject do
  use Mix.Project
  def project, do: [app: :resilient_http, version: "0.1.0", elixir: "~> 1.19", deps: []]
...
# script/main.exs
#
# Canonical entrypoint for the resilient_http project. It wires the application
...
#
# The full implementation lives under `lib/resilient_http/` and is documented in
# the Implementation section above. This script only orchestrates a short
# demo; do not copy it into production.

defmodule Main do
  @moduledoc """
  Demo driver for `ResilientHttp` — a retry helper that supports full, equal, and decorrelated jitter on top of exponential backoff.
  ...
  @spec main() :: :ok
  def main do
    IO.puts("[resilient_http] boot ok")
    {:ok, _} = ensure_started()
    run_demo()
    IO.puts("[resilient_http] demo ok")
    :ok
  end
  ...
  defp ensure_started do
    ...
    case Application.ensure_all_started(:resilient_http) do
  ...
  defp run_demo do
    ...
    # For `resilient_http`, the interesting entry points are documented above in
...
## Why Retry with Exponential Backoff and Three Jitter Strategies matters

Mastering **Retry with Exponential Backoff and Three Jitter Strategies** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `lib/resilient_http.ex`

```elixir
defmodule ResilientHttp do
  @moduledoc """
  Reference implementation for Retry with Exponential Backoff and Three Jitter Strategies.
  ...
  @doc """
  Entry point for the resilient_http module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> ResilientHttp.run(:noop)
      :ok
...
### `test/resilient_http_test.exs`

```elixir
defmodule ResilientHttpTest do
  use ExUnit.Case, async: true

  doctest ResilientHttp

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert ResilientHttp.run(:noop) == :ok
    end
  end
end
```
