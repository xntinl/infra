...
```elixir
defmodule EcsEngine.MixProject do
  use Mix.Project
...
defmodule Main do
  @moduledoc """
  Realistic stress harness for `ecs_engine` (ECS game engine).
  ...
  def main do
    :ok = Application.ensure_all_started(:ecs_engine) |> elem(0) |> then(&(&1 == :ok && :ok || :ok))
    IO.puts("=== EcsEngine stress test ===")
  ...
  defp recovery_phase do
    ...
    case Application.stop(:ecs_engine) do
      :ok -> :ok
      _ -> :ok
    end
    {:ok, _} = Application.ensure_all_started(:ecs_engine)
  ...
  defp op(fault) do
    if :rand.uniform() < fault do
      ...
      # TODO: replace with actual ecs_engine operation
      {:ok, :ok}
    end
  end
...
EcsEngine classifies every failure on two axes: **severity** (critical vs recoverable) and **scope** (per-request vs system-wide). Critical violations halt the subsystem and page an operator; recoverable faults are retried with bounded backoff and explicit budgets.
...
|---|---|---|
| **Sustained throughput** | **10,000 entities @ 60fps** | comparable to reference system |
| **Latency p50** | below p99/4 | — |
| **Latency p99** | **16 ms** | Nystrom, Game Programming Patterns |
...
- Nystrom, Game Programming Patterns: standard reference for this class of system.
...
## Why Game Engine with ECS Architecture matters

Mastering **Game Engine with ECS Architecture** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
...
defmodule GameEngine do
  @moduledoc """
  Reference implementation for Game Engine with ECS Architecture.
  ...
  @doc """
  Entry point for the game_engine module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> GameEngine.run(:noop)
      :ok
