...
def main do
  ...
  IO.puts("Build In Memory Database")
  IO.puts("")
  
  Memdb.MVCC.start_link([])
  IO.puts("Memdb.MVCC started")
...
defmodule Main do
  @moduledoc """
  Realistic stress harness for `memdb` (in-memory MVCC DB).
  ...
  def main do
    :ok = Application.ensure_all_started(:memdb) |> elem(0) |> then(&(&1 == :ok && :ok || :ok))
    IO.puts("=== Memdb stress test ===")
  ...
  defp recovery_phase do
    ...
    case Application.stop(:memdb) do
      :ok -> :ok
      _ -> :ok
    end
    {:ok, _} = Application.ensure_all_started(:memdb)
  ...
  defp op(fault) do
    if :rand.uniform() < fault do
      ...
      # TODO: replace with actual memdb operation
      {:ok, :ok}
    end
  end
...
Memdb classifies every failure on two axes: **severity** (critical vs recoverable) and **scope** (per-request vs system-wide). Critical violations halt the subsystem and page an operator; recoverable faults are retried with bounded backoff and explicit budgets.
...
|---|---|---|
| **Sustained throughput** | **1,000,000 reads/s** | comparable to reference system |
| **Latency p50** | below p99/4 | — |
| **Latency p99** | **1 ms** | PostgreSQL MVCC paper |
...
- PostgreSQL MVCC paper: standard reference for this class of system.
...
## Why In-Memory Database with MVCC and SQL Subset matters

Mastering **In-Memory Database with MVCC and SQL Subset** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
...
defmodule Memdb do
  @moduledoc """
  Reference implementation for In-Memory Database with MVCC and SQL Subset.
  ...
  @doc """
  Entry point for the memdb module. Replace the body with the real
