...
defmodule Meshex do
  ...
  the sidecar is language-agnostic — Go, Python, Rust, Elixir services all get the same guarantees. An in-process library means N implementations to maintain and non-Elixir services....
  """
end
```
### `lib/meshex/circuit_breaker.ex`
...
defmodule Meshex.CircuitBreaker do
  @moduledoc """
  Ejercicio: Service Mesh Sidecar Proxy.
...
```
### `lib/meshex/mtls/cert_generator.ex`
...
defmodule Meshex.TracingTest do
  ...
  doctest Meshex.MTLS.CertGenerator
...
```elixir
defmodule Meshex.MixProject do
  use Mix.Project
...
defmodule Main do
  @moduledoc """
  Realistic stress harness for `meshex` (service mesh proxy (sidecar)).
  ...
  def main do
    :ok = Application.ensure_all_started(:meshex) |> elem(0) |> then(&(&1 == :ok && :ok || :ok))
    IO.puts("=== Meshex stress test ===")
  ...
  defp recovery_phase do
    ...
    case Application.stop(:meshex) do
      :ok -> :ok
      _ -> :ok
    end
    {:ok, _} = Application.ensure_all_started(:meshex)
  ...
  defp op(fault) do
    if :rand.uniform() < fault do
      ...
      # TODO: replace with actual meshex operation
      {:ok, :ok}
    end
  end
...
Meshex classifies every failure on two axes: **severity** (critical vs recoverable) and **scope** (per-request vs system-wide). Critical violations halt the subsystem and page an operator; recoverable faults are retried with bounded backoff and explicit budgets.
...
| **Latency p99** | **5 ms** | Envoy + Istio architecture |
...
- Envoy + Istio architecture: standard reference for this class of system.
...
## Why Service Mesh Sidecar Proxy matters

Mastering **Service Mesh Sidecar Proxy** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `test/meshex_test.exs`

```elixir
defmodule MeshexTest do
  use ExUnit.Case, async: true
