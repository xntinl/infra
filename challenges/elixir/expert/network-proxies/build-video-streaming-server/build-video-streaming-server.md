...
```
### `lib/hls_server/playlist/media.ex`
...
```
### `lib/hls_server/range_handler.ex`
...
defmodule HLSServer.RangeHandler do
  ...
  """
  @spec process_request(binary(), String.t() | nil) :: {:ok, map()} | {:error, 416}
  def process_request(full_content, range_header) when is_nil(range_header) do
    {:ok, %{
      status: 200,
      body: full_content,
  ...
  def process_request(full_content, "bytes=" <> range_spec) do
...
defmodule HLSServer.SegmenterTest do
  ...
  doctest HLSServer.RangeHandler
...
```elixir
defmodule Videoserv.MixProject do
  use Mix.Project
...
defmodule Main do
  @moduledoc """
  Realistic stress harness for `videoserv` (HLS/DASH streaming server).
  ...
  def main do
    :ok = Application.ensure_all_started(:videoserv) |> elem(0) |> then(&(&1 == :ok && :ok || :ok))
    IO.puts("=== Videoserv stress test ===")
  ...
  defp recovery_phase do
    ...
    case Application.stop(:videoserv) do
      :ok -> :ok
      _ -> :ok
    end
    {:ok, _} = Application.ensure_all_started(:videoserv)
  ...
  defp op(fault) do
    if :rand.uniform() < fault do
      ...
      # TODO: replace with actual videoserv operation
      {:ok, :ok}
    end
  end
...
Videoserv classifies every failure on two axes: **severity** (critical vs recoverable) and **scope** (per-request vs system-wide). Critical violations halt the subsystem and page an operator; recoverable faults are retried with bounded backoff and explicit budgets.
...
|---|---|---|
| **Sustained throughput** | **10,000 viewers** | comparable to reference system |
| **Latency p50** | below p99/4 | — |
| **Latency p99** | **100 ms** | Apple HLS RFC 8216 |
...
- Apple HLS RFC 8216: standard reference for this class of system.
...
### `test/hls_server_test.exs`

```elixir
defmodule HlsServerTest do
  use ExUnit.Case, async: true
