...
### `mix.exs`
```elixir
defmodule MiniQuery.MixProject do
  use Mix.Project
...
defmodule Main do
  def main do
    IO.puts("=== MiniQuery: demo ===\n")

    result_1 = MiniQuery.run(@users)
    IO.puts("Demo 1: #{inspect(result_1)}")

    result_2 = MiniQuery.run(@users, select: [:name])
...
## Why Keyword Lists and Building a Small DSL matters

Mastering **Keyword Lists and Building a Small DSL** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
...
defmodule MiniQuery do
  @moduledoc """
  Reference implementation for Keyword Lists and Building a Small DSL.
  ...
  @doc """
  Entry point for the mini_query module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> MiniQuery.run(:noop)
      :ok
