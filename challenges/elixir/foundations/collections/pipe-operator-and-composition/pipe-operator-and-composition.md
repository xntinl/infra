...
defmodule Main do
  def main do
    ...
    result_1 = Etl.extract("name,age\\nAlice,30\\nBob,25")
    IO.puts("Demo 1: #{inspect(result_1)}")

    result_2 = Etl.extract("")
    IO.puts("Demo 2: #{inspect(result_2)}")

    result_3 = Etl.extract("name,age\\nAlice,30\\nBob,25")
...
## Why Pipe Operator and Composition matters

Mastering **Pipe Operator and Composition** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
