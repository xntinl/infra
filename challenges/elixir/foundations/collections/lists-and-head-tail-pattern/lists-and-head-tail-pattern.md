...
### `mix.exs`
```elixir
defmodule MdLite.MixProject do
  use Mix.Project
...
defmodule Main do
  def main do
    ...
    result_1 = MdLite.to_html("# Hello")
    IO.puts("Demo 1 - h1: #{inspect(result_1)}")

    result_2 = MdLite.to_html("## World")
    IO.puts("Demo 2 - h2: #{inspect(result_2)}")

    result_3 = MdLite.to_html("- apple\n- banana")
    IO.puts("Demo 3 - list: #{inspect(result_3)}")
...
## Why Lists and Head/Tail matters

Mastering **Lists and Head/Tail** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
