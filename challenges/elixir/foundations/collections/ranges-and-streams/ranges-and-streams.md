...
defmodule Main do
  def main do
    IO.puts("=== PageStream: demo ===\n")

    # Lazy range: only the first 3 elements get computed
    result_1 = 1..1_000_000 |> Stream.map(&(&1 * 2)) |> Enum.take(3)
    IO.puts("Demo 1 - lazy range, first 3 doubled: #{inspect(result_1)}")

    # Stream pipeline composes without intermediate allocations
    result_2 = Stream.iterate(1, &(&1 + 1)) |> Stream.filter(&(rem(&1, 7) == 0)) |> Enum.take(5)
    IO.puts("Demo 2 - first 5 multiples of 7: #{inspect(result_2)}")

    # Range as Enumerable (no list materialization)
    result_3 = Enum.sum(1..1_000_000)
    IO.puts("Demo 3 - sum 1..1_000_000: #{inspect(result_3)}")
...
## Why Ranges and Streams matters

Mastering **Ranges and Streams** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
