...
### `mix.exs`
```elixir
defmodule VisitorTracker.MixProject do
  use Mix.Project
...
defmodule Main do
  def main do
    IO.puts("=== VisitorTracker: demo ===\n")

    result_1 = MapSet.size(y)
    IO.puts("Demo 1: #{inspect(result_1)}")

    result_2 = VisitorTracker.returning(y, t)
    IO.puts("Demo 2: #{inspect(result_2)}")

    result_3 = VisitorTracker.new_today(y, t)
...
## Why MapSet Operations for Set Arithmetic matters

Mastering **MapSet Operations for Set Arithmetic** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `lib/visitor_tracker.ex`

```elixir
defmodule VisitorTracker do
  @moduledoc """
  Reference implementation for MapSet Operations for Set Arithmetic.
  ...
  @doc """
  Entry point for the visitor_tracker module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> VisitorTracker.run(:noop)
      :ok
...
defmodule VisitorTrackerTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert VisitorTracker.run(:noop) == :ok
    end
  end
end
```
