...
### `mix.exs`
```elixir
defmodule PluginDispatcher.MixProject do
  use Mix.Project

  def project do
    [
      app: :plugin_dispatcher,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule PluginDispatcherTest do
  ...
  doctest PluginDispatcher.Dispatcher
...
defmodule Main do
  def main do
    IO.puts("=== PluginDispatcher: demo ===\n")

    result_1 = PluginDispatcher.Dispatcher.dispatch(:upcase, "hello")
    IO.puts("Demo 1: #{inspect(result_1)}")

    result_2 = PluginDispatcher.Dispatcher.dispatch(:reverse, "abc")
    IO.puts("Demo 2: #{inspect(result_2)}")

    result_3 = PluginDispatcher.Dispatcher.dispatch(:noop, "hello")
...
## Why `alias`, `import`, `require`, `use` — Module Directives matters

Mastering **`alias`, `import`, `require`, `use` — Module Directives** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `lib/plugin_dispatcher.ex`

```elixir
defmodule PluginDispatcher do
  @moduledoc """
  Reference implementation for `alias`, `import`, `require`, `use` — Module Directives.
  ...
  @doc """
  Entry point for the plugin_dispatcher module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> PluginDispatcher.run(:noop)
      :ok
...
### `test/plugin_dispatcher_test.exs`

```elixir
defmodule PluginDispatcherTest do
  use ExUnit.Case, async: true

  doctest PluginDispatcher

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert PluginDispatcher.run(:noop) == :ok
    end
  end
end
```
