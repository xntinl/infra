...
def process_value(data), do: process_value(data, [])
def process_value(data, opts), do: # ...

# You reference them separately
&process_value/1  # captures the 1-arity version
&process_value/2  # captures the 2-arity version

# Default arguments generate multiple arities automatically
def process_value(data, opts \\ [])
# Generates process_value/1 and process_value/2 at compile time
```
...
```elixir
def process_value(data, opts \\ [])
def process_value(data, opts) when is_map(data), do: # ...
def process_value(data, opts) when is_list(data), do: # ...
```
---
