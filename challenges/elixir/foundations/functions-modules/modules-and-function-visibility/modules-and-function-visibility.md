...
### `mix.exs`
```elixir
defmodule PayAdapter.MixProject do
  use Mix.Project
...
defmodule Main do
  def main do
    IO.puts("=== PayAdapter: demo ===\n")

    result_1 = PayAdapter.charge("pm_visa_4242", %{amount: 2500, currency: "usd", description: "Order #123"})
    IO.puts("Demo 1: #{inspect(result_1)}")

    result_2 = PayAdapter.refund("ch_abc123", %{amount: 1000, reason: "customer_request"})
    IO.puts("Demo 2: #{inspect(result_2)}")

    result_3 = PayAdapter.status("ch_abc123")
...
## Why Modules and Function Visibility matters

Mastering **Modules and Function Visibility** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
