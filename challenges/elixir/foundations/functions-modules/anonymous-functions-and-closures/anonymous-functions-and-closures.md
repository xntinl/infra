...
defmodule Main do
  def main do
    IO.puts("=== RulesEngine: demo ===\n")

    result_1 = RulesEngine.evaluate(config_rules, subject)
    IO.puts("Demo 1: #{inspect(result_1)}")

    result_2 = Mix.env()
    IO.puts("Demo 2: #{inspect(result_2)}")

    result_3 = RulesEngine.min_value(:age, 18)
...
## Why Anonymous Functions and Closures matters

Mastering **Anonymous Functions and Closures** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
