...
```
### `lib/domain_display.ex`

```elixir
defmodule DomainDisplay do
  @moduledoc """
  String.Chars and Inspect Protocols: Domain Types That Display Themselves.
...
defmodule Main do
  def main do
    IO.puts("=== DomainDisplay: demo ===\n")

    result_1 = DomainDisplay.Email.to_plain(email)
    IO.puts("Demo 1: #{inspect(result_1)}")

    result_2 = DomainDisplay.UserId.value(uid)
...
## Why String.Chars and Inspect Protocols matters

Mastering **String.Chars and Inspect Protocols** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
