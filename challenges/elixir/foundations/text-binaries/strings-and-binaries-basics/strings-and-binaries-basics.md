...
defmodule LogParser do
  ...
  @doc "Extracts method result from _."
  ...
  @doc "Returns whether internal ip holds."
  ...
  @doc "Returns whether internal ip holds from _."
  ...
  @doc "Returns status category result from code."
  ...
  @doc "Returns status category result from _."
...
defmodule Main do
  def main do
    IO.puts("=== LogParser: demo ===\n")

    result_1 = String.length("José")
    IO.puts("Demo 1: #{inspect(result_1)}")

    result_2 = LogParser.extract_method("GET /api/users HTTP/1.1")
    IO.puts("Demo 2: #{inspect(result_2)}")

    result_3 = LogParser.extract_method("POST /api/users HTTP/1.1")
...
## Why Strings and Binaries matters

Mastering **Strings and Binaries** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
