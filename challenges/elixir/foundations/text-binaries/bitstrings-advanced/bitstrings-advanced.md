...
**Project**: `bitstrings_binarios_avanzados` — Bitstrings and Advanced Binaries
...
### `lib/bitstrings_binarios_avanzados.ex`

```elixir
defmodule BitstringsBinariosAvanzados do
  @moduledoc """
  Bitstrings and Advanced Binaries.
...
### `test/bitstrings_binarios_avanzados_test.exs`
...
defmodule Main do
  def main do
    IO.puts("=== BitstringsBinariosAvanzados: demo ===\n")

    result_1 = TaskQueue.Protocol.decode(frame)
    IO.puts("Demo 1: #{inspect(result_1)}")

    result_2 = TaskQueue.Protocol.decode(<<0, 1, 2, 3>>)
    IO.puts("Demo 2: #{inspect(result_2)}")

    result_3 = TaskQueue.Protocol.codepoint_count("cafe")
...
## Why Bitstrings and Advanced Binaries matters

Mastering **Bitstrings and Advanced Binaries** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
