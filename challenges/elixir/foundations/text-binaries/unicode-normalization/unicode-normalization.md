...
### `mix.exs`
```elixir
defmodule Slugger.MixProject do
  use Mix.Project
...
defmodule Main do
  def main do
    IO.puts("=== Slugger: demo ===\n")

    result_1 = Slugger.slugify("Cañón")
    IO.puts("Demo 1: #{inspect(result_1)}")

    result_2 = Slugger.slugify("café")
    IO.puts("Demo 2: #{inspect(result_2)}")

    result_3 = Slugger.slugify("São Paulo")
...
## Why Unicode Normalization and Slug Generation matters

Mastering **Unicode Normalization and Slug Generation** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
...
defmodule Slugger do
  @moduledoc """
  Reference implementation for Unicode Normalization and Slug Generation.
  ...
  @doc """
  Entry point for the slugger module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> Slugger.run(:noop)
      :ok
