...
### `mix.exs`
```elixir
defmodule SignedUrl.MixProject do
  use Mix.Project
...
defmodule Main do
  def main do
    IO.puts("=== SignedUrl: demo ===\n")

    result_1 = SignedUrl.verify(url, @secret, fn -> exp - 1 end)
    IO.puts("Demo 1: #{inspect(result_1)}")

    result_2 = SignedUrl.verify(url, @secret, fn -> exp + 1 end)
    IO.puts("Demo 2: #{inspect(result_2)}")

    result_3 = SignedUrl.verify(tampered, @secret)
...
## Why Base64URL, URI Encoding, and Signed URLs matters

Mastering **Base64URL, URI Encoding, and Signed URLs** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
...
defmodule SignedUrl do
  @moduledoc """
  Reference implementation for Base64URL, URI Encoding, and Signed URLs.
  ...
  @doc """
  Entry point for the signed_url module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> SignedUrl.run(:noop)
      :ok
