...
**Project**: `pattern_matching_avanzado` — Advanced Pattern Matching
...
**Option A — multi-clause functions with guards** (chosen)
- Pros: Pattern is visible in the signature, exhaustiveness check by the compiler, easy to extend, first-match semantics are clear
- Cons: More total lines of code when clauses share logic (solved with private helper functions)

**Option B — single-clause function with nested `if`/`cond`**
- Pros: All logic in one place, easy to spot shared computations
- Cons: Deep nesting, hard to test each branch in isolation, no exhaustiveness check, fragile to maintenance

→ Chose **A** because advanced pattern matching exists precisely to turn nested conditionals into compiler-checked dispatch tables. Multi-clause functions are the idiomatic Elixir solution for routing and branching logic.

## Implementation

### `mix.exs`
```elixir
defmodule PatternMatchingAvanzado.MixProject do
  use Mix.Project

  def project do
    [
      app: :pattern_matching_avanzado,
      version: "0.1.0",
      elixir: "~> 1.19",
...
```
### `lib/pattern_matching_avanzado.ex`

```elixir
defmodule PatternMatchingAvanzado do
  @moduledoc """
  Advanced Pattern Matching.
...
### `test/pattern_matching_avanzado_test.exs`
```elixir
defmodule TaskQueue.PatternMatchingTest do
  use ExUnit.Case, async: true
  doctest TaskQueue.PayloadDecoder
...
defmodule Main do
  def main do
    IO.puts("=== PatternMatchingAvanzado: demo ===\n")
...
```elixir
def process_value({:ok, result}), do: result
def process_value({:error, reason}), do: {:error, reason}
```
