...
### `mix.exs`
```elixir
defmodule WebhookEventRouter.MixProject do
  use Mix.Project

  def project do
    [
      app: :webhook_event_router,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule Main do
  def main do
    IO.puts("=== WebhookEventRouter: demo ===\n")
...
## Why Map Pattern Matching matters

Mastering **Map Pattern Matching** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
...
```elixir
def process_value(%{age: age} = person) when age >= 18, do: person
```
