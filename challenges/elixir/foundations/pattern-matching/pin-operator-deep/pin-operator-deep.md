...
### `mix.exs`
```elixir
defmodule PinnedEventDispatcher.MixProject do
  use Mix.Project

  def project do
    [
      app: :pinned_event_dispatcher,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule Main do
  def main do
    IO.puts("=== PinnedEventDispatcher: demo ===\n")

    result_1 = PinnedEventDispatcher.dispatch(@event, :click, "u-1")
    IO.puts("Demo 1: #{inspect(result_1)}")

    result_2 = PinnedEventDispatcher.dispatch(@event, :click, "u-2")
    IO.puts("Demo 2: #{inspect(result_2)}")

    result_3 = PinnedEventDispatcher.dispatch(@event, :scroll, "u-1")
...
## Why The Pin Operator in Depth matters

Mastering **The Pin Operator in Depth** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
