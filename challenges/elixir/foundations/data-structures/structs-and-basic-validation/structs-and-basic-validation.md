...
### `mix.exs`
```elixir
defmodule UserSchema.MixProject do
  use Mix.Project
...
defmodule Main do
  def main do
    IO.puts("=== UserSchema: demo ===\n")

    result_1 = Map.has_key?(errors, :email)
    IO.puts("Demo 1: #{inspect(result_1)}")

    result_2 = UserSchema.admin?(user)
...
## Why Structs and Validation matters

Mastering **Structs and Validation** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
