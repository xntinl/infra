...
### `mix.exs`
```elixir
defmodule Jsonish.MixProject do
  use Mix.Project
...
defprotocol Jsonish do
  ...
  @doc "Encodes result from value."
  @spec encode(t) :: String.t()
  def encode(value)
end
```

### `lib/jsonish/builtins.ex`
...
defimpl Jsonish, for: Atom do
  ...
  @doc "Encodes result from nil."
  def encode(nil), do: "null"
  @doc "Encodes result from true."
  def encode(true), do: "true"
  @doc "Encodes result from false."
  def encode(false), do: "false"
  @doc "Encodes result from atom."
...
defmodule Main do
  def main do
    IO.puts("=== Jsonish: demo ===\n")

    result_1 = Jsonish.encode(42)
    IO.puts("Demo 1: #{inspect(result_1)}")

    result_2 = Jsonish.encode(~s(hello "world"))
    IO.puts("Demo 2: #{inspect(result_2)}")

    result_3 = Jsonish.encode(nil)
...
## Why Protocols for Runtime Polymorphism matters

Mastering **Protocols for Runtime Polymorphism** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `test/jsonish_test.exs`
