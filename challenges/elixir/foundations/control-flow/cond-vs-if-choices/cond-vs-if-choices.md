...
### `mix.exs`
```elixir
defmodule PricingRuleEngine.MixProject do
  use Mix.Project

  def project do
    [
      app: :pricing_rule_engine,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule PricingRuleEngineTest do
  ...
  doctest PricingRuleEngine.Pricing
...
defmodule Main do
  def main do
    IO.puts("=== PricingRuleEngine: demo ===\n")

    result_1 = PricingRuleEngine.Pricing.total(%{subtotal: 50, qty: 1, tax_exempt: false})
    IO.puts("Demo 1: #{inspect(result_1)}")

    result_2 = PricingRuleEngine.Pricing.total(%{subtotal: 100, qty: 1, tax_exempt: false})
    IO.puts("Demo 2: #{inspect(result_2)}")

    result_3 = PricingRuleEngine.Pricing.total(%{subtotal: 500, qty: 1, tax_exempt: false})
...
## Why `cond` vs nested `if` — when each one fits matters

Mastering **`cond` vs nested `if` — when each one fits** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `lib/pricing_rule_engine.ex`

```elixir
defmodule PricingRuleEngine do
  @moduledoc """
  Reference implementation for `cond` vs nested `if` — when each one fits.
  ...
  @doc """
  Entry point for the pricing_rule_engine module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> PricingRuleEngine.run(:noop)
      :ok
...
### `test/pricing_rule_engine_test.exs`

```elixir
defmodule PricingRuleEngineTest do
  use ExUnit.Case, async: true

  doctest PricingRuleEngine

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert PricingRuleEngine.run(:noop) == :ok
    end
  end
end
```
