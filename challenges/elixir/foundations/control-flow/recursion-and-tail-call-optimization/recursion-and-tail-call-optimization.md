...
**Option A — tail-recursive work-list pattern with explicit accumulator** (chosen)
- Pros: Constant stack usage regardless of tree depth, safe on very deep trees (100k+ files)
- Cons: Requires explicit work-list management; adds complexity to understand at first

**Option B — body-recursive (stack-accumulating) version**
- Pros: Reads closer to the mathematical definition, simpler to reason about
- Cons: Stack grows linearly with depth; on a 100k-deep path, **stack overflow or OOM**

→ Chose **A** because a file-system walker can encounter arbitrarily deep trees — tail-call optimization is not optional. The work-list pattern is the standard solution to maintain tail recursion while traversing trees.
...
defmodule Main do
  def main do
    ...
    result_1 = Tree.total_size("/tmp/tree_test_dir")
    IO.puts("Demo 1: #{inspect(result_1)}")

    result_2 = Tree.format(entries)
    IO.puts("Demo 2: #{inspect(result_2)}")

    result_3 = Tree.count_by_extension(entries)
...
## Why Recursion and Tail Call Optimization matters

Mastering **Recursion and Tail Call Optimization** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
...
A tail-recursive call is one where the recursive call is the **last** operation — the return value of the function is exactly the return value of the recursive call.

**BEAM reuses the stack frame WHEN:**
- The recursive call is in tail position (nothing happens after it)
- The function can be compiled to jump (not call) the next clause
- No `try`/`rescue` wraps the recursive call

**TCO breaks (stack grows) WHEN:**
- Operation happens AFTER recursion: `1 + factorial(n-1)` — the `+` is after the recursive call
- `try`/`rescue` wraps the recursive call (error handling breaks tail position)
- Function returns a compound structure built with the recursive result

Examples that FAIL TCO (stack grows):
```elixir
# BAD: addition after recursion
def sum([]), do: 0
def sum([h|t]), do: h + sum(t)  # stack overflow at ~10k elements

# BAD: try breaks tail position
def safe_sum([h|t]) do
  try do
    sum(t)
  rescue
...
# BAD: building list with recursive result
def flatten([[h|t]|rest]), do: [h | flatten([t|rest])]
```
Examples that KEEP TCO (stack reuses):
```elixir
# GOOD: recursive call is last operation
def sum(list), do: sum(list, 0)
defp sum([], acc), do: acc
defp sum([h|t], acc), do: sum(t, h + acc)  # works on 1M+ elements

# GOOD: error handling outside recursion
...
defp sum_tail([h|t], acc), do: sum_tail(t, h + acc)
```
