...
### `mix.exs`
```elixir
defmodule VersionedSchemaRegistry.MixProject do
  use Mix.Project

  def project do
    [
      app: :versioned_schema_registry,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule VersionedSchemaRegistryTest do
  ...
  doctest VersionedSchemaRegistry
...
defmodule Main do
  def main do
    IO.puts("=== VersionedSchemaRegistry: demo ===\n")

    result_1 = VersionedSchemaRegistry.current_version()
    IO.puts("Demo 1: #{inspect(result_1)}")

    result_2 = VersionedSchemaRegistry.lag("3.2.0")
    IO.puts("Demo 2: #{inspect(result_2)}")

    result_3 = VersionedSchemaRegistry.lag("3.1.0")
...
## Why Module Attributes and Constants matters

Mastering **Module Attributes and Constants** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.
