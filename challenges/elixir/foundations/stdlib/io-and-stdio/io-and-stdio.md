...
defmodule Main do
  def main do
    IO.puts("=== CalcRepl: demo ===\n")

    # Evaluator works without IO — easy to demo
    result_1 = CalcRepl.Evaluator.evaluate("2 + 3 * 4")
    IO.puts("Demo 1 - precedence: #{inspect(result_1)}")

    result_2 = CalcRepl.Evaluator.evaluate("(10 - 4) / 2")
    IO.puts("Demo 2 - parens: #{inspect(result_2)}")

    result_3 = CalcRepl.Evaluator.evaluate("nope!")
    IO.puts("Demo 3 - invalid input: #{inspect(result_3)}")
