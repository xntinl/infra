...
### `mix.exs`
```elixir
defmodule UrlValidator.MixProject do
  use Mix.Project
...
defmodule Main do
  def main do
    IO.puts("=== UrlValidator: demo ===\n")

    result_1 = URI.parse("not a url")
...
## Why URI parsing and normalization matters

Mastering **URI parsing and normalization** directly impacts how you design reliable, scalable Elixir systems. The patterns and trade-offs covered in this exercise appear in production code shipped by companies running the BEAM at scale — from payment processors to chat platforms to telemetry pipelines.

Understanding the underlying semantics (not just the syntax) is what separates engineers who can debug a cascading failure at 3 AM from those who can only write new code. This document focuses on the *why*: memory layout, process boundaries, failure semantics, and the trade-offs you are implicitly accepting when you choose one approach over another.

Invest the time here. The compound interest on fundamental concepts is enormous.

### `test/url_validator_test.exs`
...
defmodule UrlValidatorTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert UrlValidator.run(:noop) == :ok
    end
  end
end
```
