...
## Why protocol vs behaviour matters
...
- Pros: Less ceremony; the protocol vs behaviour flow fits in a single short module.
- Cons: Reinvents supervision, restart, back-pressure, and observability — the four properties OTP gives us for free.

**Option B — Use the canonical OTP shape for protocol vs behaviour** (chosen)
...
```elixir
defmodule ProtoVsBehaviour.MixProject do
  use Mix.Project

  def project do
    [
      app: :proto_vs_behaviour,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defprotocol Shape do
  ...
  @doc "Returns area result from shape."
  @spec area(t) :: float()
  def area(shape)
end
```
...
defmodule ShapeCalc.Rectangle do
  ...
  @doc "Returns area result from h."
  @impl ShapeCalc
...
### `test/proto_vs_behaviour_test.exs`
...
defmodule ProtoVsBehaviourTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert ProtoVsBehaviour.run(:noop) == :ok
    end
  end
end
```
