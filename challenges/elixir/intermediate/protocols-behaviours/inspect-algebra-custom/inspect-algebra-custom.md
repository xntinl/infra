...
## Why inspect algebra custom matters
...
- Pros: Less ceremony; the inspect algebra custom flow fits in a single short module.
- Cons: Reinvents supervision, restart, back-pressure, and observability — the four properties OTP gives us for free.

**Option B — Use the canonical OTP shape for inspect algebra custom** (chosen)
...
defimpl Inspect, for: LineItem do
  ...
  @doc "Returns inspect result from qty, price and opts."
...
defimpl Inspect, for: Order do
  ...
  @doc "Returns inspect result from customer, items and opts."
...
defmodule PrettyInspectTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert PrettyInspect.run(:noop) == :ok
    end
  end
end
```
