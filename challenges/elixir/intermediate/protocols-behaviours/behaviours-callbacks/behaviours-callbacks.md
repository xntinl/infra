...
## Why behaviours y callbacks matters
...
storage_behaviour/
├── lib/
│   └── storage_behaviour.ex
├── script/
│   └── main.exs
├── test/
│   └── storage_behaviour_test.exs
└── mix.exs
```
...
```elixir
defmodule StorageBehaviour.MixProject do
  use Mix.Project

  def project do
    [
      app: :storage_behaviour,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule Main do
  @moduledoc """
  Runnable demo of `StorageBehaviour`.
...
### `test/storage_behaviour_test.exs`

```elixir
defmodule StorageBehaviourTest do
  use ExUnit.Case, async: true

  doctest StorageBehaviour

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert StorageBehaviour.run(:noop) == :ok
    end
  end
end
```