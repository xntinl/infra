...
## Why protocol fallback any matters
...
fallback_any_demo/
├── lib/
│   └── fallback_any_demo.ex
├── script/
│   └── main.exs
├── test/
│   └── fallback_any_demo_test.exs
└── mix.exs
```
...
```elixir
defmodule FallbackAnyDemo.MixProject do
  use Mix.Project

  def project do
    [
      app: :fallback_any_demo,
      version: "0.1.0",
      elixir: "~> 1.19",
...
### `test/fallback_any_demo_test.exs`

```elixir
defmodule FallbackAnyDemoTest do
  use ExUnit.Case, async: true

  doctest FallbackAnyDemo

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert FallbackAnyDemo.run(:noop) == :ok
    end
  end
end
```
