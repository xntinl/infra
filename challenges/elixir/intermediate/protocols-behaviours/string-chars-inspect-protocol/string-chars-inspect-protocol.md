...
## Why string chars inspect protocol matters
...
```elixir
defmodule PrettyMoney.MixProject do
  use Mix.Project
...
defmodule Main do
  def main do
    ...
    result_1 = Money.new(nil, nil)
    IO.puts("Demo 1 - new: #{inspect(result_1)}")
    result_2 = Money.format_amount(nil)
    IO.puts("Demo 2 - format_amount: #{inspect(result_2)}")
    result_3 = Money.to_string(nil)
    IO.puts("Demo 3 - to_string: #{inspect(result_3)}")
  end
end
...
### `test/pretty_money_test.exs`

```elixir
defmodule PrettyMoneyTest do
  use ExUnit.Case, async: true
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert PrettyMoney.run(:noop) == :ok
    end
  end
end
```
