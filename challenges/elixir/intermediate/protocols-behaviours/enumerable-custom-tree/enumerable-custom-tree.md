...
## Why enumerable custom tree matters
...
- Pros: Less ceremony; the enumerable custom tree flow fits in a single short module.
- Cons: Reinvents supervision, restart, back-pressure, and observability — the four properties OTP gives us for free.

**Option B — Use the canonical OTP shape for enumerable custom tree** (chosen)
...
```elixir
defmodule TreeEnum.MixProject do
  use Mix.Project
...
### `test/tree_enum_test.exs`

```elixir
defmodule TreeEnumTest do
  use ExUnit.Case, async: true
