...
```elixir
defmodule JsonableProto.MixProject do
  use Mix.Project
...
defmodule Main do
  @moduledoc """
  Runnable demo of `JsonableProto`.
...
### `test/jsonable_proto_test.exs`

```elixir
defmodule JsonableProtoTest do
  use ExUnit.Case, async: true

  doctest JsonableProto

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert JsonableProto.run(:noop) == :ok
    end
  end
end
```
