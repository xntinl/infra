...
```elixir
defmodule MyGenerator.MixProject do
  use Mix.Project
...
### `lib/mix/tasks/my_generator.worker.ex`
...
defmodule Mix.Tasks.MyGenerator.WorkerTest do
  ...
  doctest Mix.Tasks.MyGenerator.Worker
...
### `test/my_generator_test.exs`

```elixir
defmodule MyGeneratorTest do
  use ExUnit.Case, async: true
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert MyGenerator.run(:noop) == :ok
    end
  end
end
```
