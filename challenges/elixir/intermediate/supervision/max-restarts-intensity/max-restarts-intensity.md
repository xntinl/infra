...
## Why max restarts intensity matters
...
```elixir
defmodule RestartIntensityDemo.MixProject do
  use Mix.Project

  def project do
    [
      app: :restart_intensity_demo,
      version: "0.1.0",
      elixir: "~> 1.19",
...
### `lib/restart_intensity_demo.ex`

```elixir
defmodule RestartIntensityDemo do
  @moduledoc """
  `max_restarts` and `max_seconds` — crash-loop detection.
...
### `lib/restart_intensity_demo/flaky_worker.ex`

**Objective**: Implement `flaky_worker.ex` — a worker whose crash behavior is the whole point — it exists so the supervisor strategy can be observed.
...
### `lib/restart_intensity_demo/supervisor.ex`
...
**Objective**: Write `restart_intensity_demo_test.exs` — tests pin the behaviour so future refactors cannot silently regress the invariants established above.

```elixir
defmodule RestartIntensityDemoTest do
  use ExUnit.Case, async: false

  doctest RestartIntensityDemo
...
defmodule Main do
  @moduledoc """
  Runnable demo of `RestartIntensityDemo`.
  ...
  def main do
    IO.puts("=== RestartIntensityDemo demo ===")
    run()
...
### `test/restart_intensity_demo_test.exs`
...
defmodule RestartIntensityDemoTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert RestartIntensityDemo.run(:noop) == :ok
    end
  end
end
```
