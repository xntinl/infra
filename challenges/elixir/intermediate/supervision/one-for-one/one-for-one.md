...
```elixir
defmodule OneForOneDemo.MixProject do
  use Mix.Project

  def project do
    [
      app: :one_for_one_demo,
      version: "0.1.0",
      elixir: "~> 1.19",
...
### `lib/one_for_one_demo.ex`

```elixir
defmodule OneForOneDemo do
  @moduledoc """
  `:one_for_one` — isolate sibling failures.
...
### `lib/one_for_one_demo/counter.ex`
...
### `lib/one_for_one_demo/supervisor.ex`
...
defmodule Main do
  @moduledoc """
  Runnable demo of `OneForOneDemo`.
...
### `test/one_for_one_demo_test.exs`
...
defmodule OneForOneDemoTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert OneForOneDemo.run(:noop) == :ok
    end
  end
end
```
