...
```elixir
defmodule ShutdownTimeoutsDemo.MixProject do
  use Mix.Project

  def project do
    [
      app: :shutdown_timeouts_demo,
      version: "0.1.0",
      elixir: "~> 1.19",
...
### `lib/shutdown_timeouts_demo.ex`

```elixir
defmodule ShutdownTimeoutsDemo do
  @moduledoc """
  `:shutdown` — graceful termination, timeouts, and `terminate/2`.
...
### `lib/shutdown_timeouts_demo/slow_worker.ex`
...
### `lib/shutdown_timeouts_demo/supervisor.ex`
...
### `test/shutdown_timeouts_demo_test.exs`
...
defmodule ShutdownTimeoutsDemoTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert ShutdownTimeoutsDemo.run(:noop) == :ok
    end
  end
end
```
