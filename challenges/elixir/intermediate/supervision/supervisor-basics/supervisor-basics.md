...
```elixir
defmodule BasicSupervisor.MixProject do
  use Mix.Project

  def project do
    [
      app: :basic_supervisor,
      version: "0.1.0",
      elixir: "~> 1.19",
...
### `lib/basic_supervisor/worker.ex`
...
### `lib/basic_supervisor/supervisor.ex`
...
defmodule Main do
  @moduledoc """
  Runnable demo of `BasicSupervisor`.
...
### `test/basic_supervisor_test.exs`
...
defmodule BasicSupervisorTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert BasicSupervisor.run(:noop) == :ok
    end
  end
end
```
