...
## Why transient temporary permanent matters
...
```elixir
defmodule RestartStrategiesDemo.MixProject do
  use Mix.Project

  def project do
    [
      app: :restart_strategies_demo,
      version: "0.1.0",
      elixir: "~> 1.19",
...
### `lib/restart_strategies_demo.ex`

```elixir
defmodule RestartStrategiesDemo do
  @moduledoc """
  `:permanent`, `:transient`, `:temporary` — picking the right restart policy.
...
### `lib/restart_strategies_demo/worker.ex`
...
### `lib/restart_strategies_demo/supervisor.ex`
...
### `test/restart_strategies_demo_test.exs`
...
defmodule RestartStrategiesDemoTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert RestartStrategiesDemo.run(:noop) == :ok
    end
  end
end
```
