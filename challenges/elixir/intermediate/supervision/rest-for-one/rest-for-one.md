...
```elixir
defmodule RestForOneDemo.MixProject do
  use Mix.Project

  def project do
    [
      app: :rest_for_one_demo,
      version: "0.1.0",
      elixir: "~> 1.19",
...
### `lib/rest_for_one_demo.ex`

```elixir
defmodule RestForOneDemo do
  @moduledoc """
  `:rest_for_one` — directional dependencies.
...
### `lib/rest_for_one_demo/stage.ex`
...
### `lib/rest_for_one_demo/supervisor.ex`
...
defmodule Main do
  @moduledoc """
  Runnable demo of `RestForOneDemo`.
...
### `test/rest_for_one_demo_test.exs`
...
defmodule RestForOneDemoTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert RestForOneDemo.run(:noop) == :ok
    end
  end
end
```
