...
## Why macros y quote unquote matters
...
```elixir
defmodule MacroBasics.MixProject do
  use Mix.Project
...
defmodule Main do
  ...
  defp run do
    # MacroBasics.unquote/1 requires 1 argument(s);
...
defmodule MacroBasicsTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert MacroBasics.run(:noop) == :ok
    end
  end
end
```
