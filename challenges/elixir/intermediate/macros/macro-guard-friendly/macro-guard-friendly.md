...
## Why macro guard friendly matters
...
```elixir
defmodule GuardMacros.MixProject do
  use Mix.Project
...
defmodule Main do
  ...
  defp run do
    # GuardMacros.validate/1 requires 1 argument(s);
    # call it with real values appropriate for this exercise.
    # GuardMacros.count_nats/1 requires 1 argument(s);
    # call it with real values appropriate for this exercise.
    :ok
  end
end
...
defmodule GuardMacrosTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert GuardMacros.run(:noop) == :ok
    end
  end
end
```
