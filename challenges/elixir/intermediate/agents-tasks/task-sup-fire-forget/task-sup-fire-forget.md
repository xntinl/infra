...
## Why task sup fire forget matters
...
```elixir
defmodule FireForgetSup.MixProject do
  use Mix.Project
...
### `lib/fire_forget_sup.ex`

```elixir
defmodule FireForgetSup do
  @moduledoc """
  Fire-and-forget with `Task.Supervisor`.
...
### `lib/fire_forget_sup/application.ex`
...
defmodule FireForgetSupTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert FireForgetSup.run(:noop) == :ok
    end
  end
end
```
