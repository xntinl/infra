...
## Why bounded concurrency pool matters
...
```elixir
defmodule BoundedPool.MixProject do
  use Mix.Project
...
### `lib/bounded_pool/application.ex`
      ...
      Task.Supervisor.start_child(@task_sup, fn ->
        result =
          try do
            ...
            e in RuntimeError -> {:exit, {e, __STACKTRACE__}}
          catch
...
defmodule BoundedPoolTest do
  ...
  describe "run/3 — happy path" do
    ...
    test "surfaces crashes", %{server: s} do
      assert {:exit, _} = BoundedPool.Server.run(s, fn -> raise ArgumentError, "boom" end)
    end
  end
...
defmodule BoundedPoolTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert BoundedPool.run(:noop) == :ok
    end
  end
end
```
