...
```elixir
defmodule TtlCacheAgent.MixProject do
  use Mix.Project
...
### `lib/ttl_cache_agent/sweeper.ex`
...
defmodule TtlCacheAgentTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert TtlCacheAgent.run(:noop) == :ok
    end
  end
end
```
