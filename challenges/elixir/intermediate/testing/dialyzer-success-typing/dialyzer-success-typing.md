...
## Why dialyzer success typing matters
...
```elixir
defmodule DialyzerDemo.MixProject do
  use Mix.Project
...
```elixir
defmodule DialyzerSuccessTyping.MixProject do
  use Mix.Project

  def project do
    [
      app: :dialyzer_success_typing,
      version: "0.1.0",
      elixir: "~> 1.19",
      start_permanent: Mix.env() == :prod,
      deps: deps()
...
defmodule Reports do
  ...
  @doc "Returns report result from invoices."
...
defmodule DialyzerDemoTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert DialyzerDemo.run(:noop) == :ok
    end
  end
end
```
