...
## Why doctests with assertions matters
...
```elixir
defmodule DoctestsDemo.MixProject do
  use Mix.Project
...
defmodule StringKit do
  ...
  @doc "Returns reverse words result from str."
  ...
  @doc "Counts occurrences result from haystack and needle."
...
### `test/doctests_demo_test.exs`

```elixir
defmodule DoctestsDemoTest do
  use ExUnit.Case, async: true

  doctest DoctestsDemo

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert DoctestsDemo.run(:noop) == :ok
    end
  end
end
```
