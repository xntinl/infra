...
## Why application callbacks matters
...
### `lib/my_service_app.ex`

```elixir
defmodule MyServiceApp do
  @moduledoc """
  Application callbacks — `start/2` and `stop/1`.
...
### `lib/my_service_app/application.ex`
...
defmodule MyServiceAppTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert MyServiceApp.run(:noop) == :ok
    end
  end
end
```
