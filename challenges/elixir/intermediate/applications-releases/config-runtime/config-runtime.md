...
## Why configuracion config runtime matters
...
```elixir
defmodule RuntimeConfigDemo.MixProject do
  use Mix.Project

  def project do
    [
      app: :runtime_config_demo,
      version: "0.1.0",
      elixir: "~> 1.19",
...
### `lib/runtime_config_demo/application.ex`
...
### `test/runtime_config_demo_test.exs`
...
defmodule RuntimeConfigDemoTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert RuntimeConfigDemo.run(:noop) == :ok
    end
  end
end
```
