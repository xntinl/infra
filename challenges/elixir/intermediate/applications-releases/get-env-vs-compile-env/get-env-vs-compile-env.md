...
## Why get env vs compile env matters
...
```elixir
defmodule EnvVsCompile.MixProject do
  use Mix.Project
...
defmodule EnvVsCompileTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert EnvVsCompile.run(:noop) == :ok
    end
  end
end
```
