...
```elixir
defmodule AppEnvRuntime.MixProject do
  use Mix.Project
...
### `lib/app_env_runtime.ex`

```elixir
defmodule AppEnvRuntime do
  @moduledoc """
  Reading config at runtime with `Application.fetch_env!/2`.
...
### `lib/app_env_runtime/application.ex`
...
defmodule Main do
  @moduledoc """
  Runnable demo of `AppEnvRuntime`.
...
defmodule AppEnvRuntimeTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert AppEnvRuntime.run(:noop) == :ok
    end
  end
end
```
