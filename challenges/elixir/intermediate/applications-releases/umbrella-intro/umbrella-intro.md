...
### `lib/tiny_umbrella.ex`

```elixir
defmodule TinyUmbrella do
  @moduledoc """
  Reference implementation for Umbrella projects — multiple apps in one tree.
  ...
  @doc """
  Entry point for the tiny_umbrella module. Replace the body with the real
  implementation once you have worked through the exercise.

  ## Examples

      iex> TinyUmbrella.run(:noop)
      :ok
...
### `test/tiny_umbrella_test.exs`

```elixir
defmodule TinyUmbrellaTest do
  use ExUnit.Case, async: true

  doctest TinyUmbrella

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert TinyUmbrella.run(:noop) == :ok
    end
  end
end
```
