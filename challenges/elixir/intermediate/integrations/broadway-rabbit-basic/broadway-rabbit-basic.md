...
## Why broadway rabbit basic matters
...
```elixir
defmodule RabbitWorker.MixProject do
  use Mix.Project
...
### `lib/rabbit_worker/application.ex`
...
defmodule RabbitWorker.Sink do
  ...
  @doc "Returns flush result from payloads."
...
### `test/rabbit_worker_test.exs`

```elixir
defmodule RabbitWorkerTest do
  use ExUnit.Case, async: true

  doctest RabbitWorker

  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert RabbitWorker.run(:noop) == :ok
    end
  end
end
```
