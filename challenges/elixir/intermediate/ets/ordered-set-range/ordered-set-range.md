...
```elixir
defmodule OrderedSetRange.MixProject do
  use Mix.Project

  def project do
    [
      app: :ordered_set_range,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule Main do
  @moduledoc """
  Runnable demo of `OrderedSetRange`.
  ...
  def main do
    IO.puts("=== OrderedSetRange demo ===")
    run()
    IO.puts("\nDone — demo finished without errors.")
  end

  defp run do
    result = OrderedSetRange.new()
    IO.inspect(result, label: "OrderedSetRange.new/0")
    :ok
  end
end
...
### `test/ordered_set_range_test.exs`
...
defmodule OrderedSetRangeTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert OrderedSetRange.run(:noop) == :ok
    end
  end
end
```
