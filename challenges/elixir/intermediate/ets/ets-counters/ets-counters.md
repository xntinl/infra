...
```elixir
defmodule EtsCountersDemo.MixProject do
  use Mix.Project

  def project do
    [
      app: :ets_counters_demo,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule Main do
  @moduledoc """
  Runnable demo of `EtsCountersDemo`.
  ...
  def main do
    IO.puts("=== EtsCountersDemo demo ===")
    run()
    IO.puts("\nDone — demo finished without errors.")
  end

  defp run do
    result = EtsCountersDemo.new_ets_table()
    IO.inspect(result, label: "EtsCountersDemo.new_ets_table/0")
    :ok
  end
end
...
### `test/ets_counters_demo_test.exs`
...
defmodule EtsCountersDemoTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert EtsCountersDemo.run(:noop) == :ok
    end
  end
end
```
