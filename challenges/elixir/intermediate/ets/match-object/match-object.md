...
```elixir
defmodule MatchObjectDemo.MixProject do
  use Mix.Project

  def project do
    [
      app: :match_object_demo,
      version: "0.1.0",
      elixir: "~> 1.19",
...
defmodule Main do
  @moduledoc """
  Runnable demo of `MatchObjectDemo`.
  ...
  def main do
    IO.puts("=== MatchObjectDemo demo ===")
    run()
    IO.puts("\nDone — demo finished without errors.")
  end

  defp run do
    result = MatchObjectDemo.seed()
    IO.inspect(result, label: "MatchObjectDemo.seed/0")
    :ok
  end
end
...
### `test/match_object_demo_test.exs`
...
defmodule MatchObjectDemoTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert MatchObjectDemo.run(:noop) == :ok
    end
  end
end
```
