...
## Why call vs cast tradeoffs matters
...
```elixir
defmodule CallVsCastDemo.MixProject do
  use Mix.Project

  def project do
    [
      app: :call_vs_cast_demo,
      version: "0.1.0",
      elixir: "~> 1.19",
...
### `lib/call_vs_cast_demo/sync_server.ex`
...
### `lib/call_vs_cast_demo/async_server.ex`
...
### `test/call_vs_cast_demo_test.exs`
...
defmodule CallVsCastDemoTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert CallVsCastDemo.run(:noop) == :ok
    end
  end
end
```
