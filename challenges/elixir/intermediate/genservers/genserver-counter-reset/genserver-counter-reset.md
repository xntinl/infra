...
## Why genserver counter reset matters
...
```elixir
defmodule CounterResetGs.MixProject do
  use Mix.Project

  def project do
    [
      app: :counter_reset_gs,
      version: "0.1.0",
      elixir: "~> 1.19",
...
### `test/counter_reset_gs_test.exs`
...
defmodule CounterResetGsTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert CounterResetGs.run(:noop) == :ok
    end
  end
end
```
