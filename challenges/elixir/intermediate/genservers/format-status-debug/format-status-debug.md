...
## Why format status debug matters
...
```elixir
defmodule FormatStatusGs.MixProject do
  use Mix.Project

  def project do
    [
      app: :format_status_gs,
      version: "0.1.0",
      elixir: "~> 1.19",
...
### `test/format_status_gs_test.exs`
...
defmodule FormatStatusGsTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert FormatStatusGs.run(:noop) == :ok
    end
  end
end
```
