...
## Why process dictionary erlang matters
...
```elixir
defmodule ProcessDictLab.MixProject do
  use Mix.Project

  def project do
    [
      app: :process_dict_lab,
      version: "0.1.0",
      elixir: "~> 1.19",
...
### `lib/process_dict_lab/implicit.ex`
...
```
### `lib/process_dict_lab/explicit.ex`
...
### `test/process_dict_lab_test.exs`
...
defmodule ProcessDictLabTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert ProcessDictLab.run(:noop) == :ok
    end
  end
end
```
