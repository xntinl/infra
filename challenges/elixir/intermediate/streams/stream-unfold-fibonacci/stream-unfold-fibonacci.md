...
## Why stream unfold fibonacci matters
...
```elixir
defmodule UnfoldFib.MixProject do
  use Mix.Project
...
defmodule Main do
  ...
  def main do
    IO.puts("=== UnfoldFib demo ===")
    run()
    IO.puts("\nDone — demo finished without errors.")
  end

  defp run do
    result = UnfoldFib.fibonacci()
    IO.inspect(result, label: "UnfoldFib.fibonacci/0")
    :ok
  end
end
