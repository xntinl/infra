...
## Why genstage producer consumer matters
...
- Pros: Less ceremony; the genstage producer consumer flow fits in a single short module.
- Cons: Reinvents supervision, restart, back-pressure, and observability — the four properties OTP gives us for free.

**Option B — Use the canonical OTP shape for genstage producer consumer** (chosen)
...
### `lib/producer_consumer/producer.ex`
...
defmodule ProducerConsumer.Producer do
  ...
  @doc "Handles demand result from demand and counter."
  @impl true
...
### `lib/producer_consumer/squarer.ex`
...
### `lib/producer_consumer/consumer.ex`
...
defmodule ProducerConsumer.Consumer do
  ...
  @doc "Handles events result from events, _from and notify_pid."
  @impl true
...
defmodule ProducerConsumer do
  ...
  @doc "Starts pipeline result from notify_pid."
...
### `test/producer_consumer_test.exs`
...
defmodule ProducerConsumerTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert ProducerConsumer.run(:noop) == :ok
    end
  end
end
```
