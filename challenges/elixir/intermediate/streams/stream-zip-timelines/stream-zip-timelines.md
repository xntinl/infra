...
## Why stream zip timelines matters
...
```elixir
defmodule ZipTimelines.MixProject do
  use Mix.Project
...
defmodule Main do
  ...
  def main do
    IO.puts("=== ZipTimelines demo ===")
    run()
    IO.puts("\nDone — demo finished without errors.")
  end

  defp run do
    # ZipTimelines.paired/2 requires 2 argument(s);
    # call it with real values appropriate for this exercise.
    # ZipTimelines.paired_n/1 requires 1 argument(s);
    # call it with real values appropriate for this exercise.
    :ok
  end
end
...
defmodule ZipTimelinesTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert ZipTimelines.run(:noop) == :ok
    end
  end
end
```
