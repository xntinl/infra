...
```elixir
defmodule ChatRoomsRegistry.MixProject do
  use Mix.Project

  def project do
    [
      app: :chat_rooms_registry,
      version: "0.1.0",
      elixir: "~> 1.19",
...
### `lib/chat_rooms_registry.ex`

```elixir
defmodule ChatRoomsRegistry do
  @moduledoc """
  Dynamic Registry for named chat rooms.
...
### `lib/chat_rooms_registry/application.ex`
...
### `lib/chat_rooms_registry/room.ex`
...
### `lib/chat_rooms_registry/rooms.ex`
...
defmodule Main do
  @moduledoc """
  Runnable demo of `ChatRoomsRegistry`.
...
### `test/chat_rooms_registry_test.exs`
...
defmodule ChatRoomsRegistryTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert ChatRoomsRegistry.run(:noop) == :ok
    end
  end
end
```
