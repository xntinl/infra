...
## Why naming strategies review matters
...
```elixir
defmodule NamingReview.MixProject do
  use Mix.Project
...
defmodule NamingReview.Counter do
  ...
  @doc "Returns bump result from server."
...
### `lib/naming_review/application.ex`
...
### `lib/naming_review.ex`

**Objective**: Implement `naming_review.ex` — the naming/lookup strategy that decides how processes are addressed under concurrency and failure.
...
defmodule NamingReview do
  ...
  @doc "Starts atom result from name."
  ...
  @doc "Starts via result from key."
  ...
  @doc "Starts global result from name."
  ...
  @doc "Starts in group result from group."
...
**Objective**: Write `naming_review_test.exs` — tests pin the behaviour so future refactors cannot silently regress the invariants established above.

```elixir
defmodule NamingReviewTest do
  use ExUnit.Case, async: false

  doctest NamingReview
...
defmodule NamingReviewTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert NamingReview.run(:noop) == :ok
    end
  end
end
```
