...
## Why registry partitioned matters
...
```elixir
defmodule PartitionedReg.MixProject do
  use Mix.Project
...
defmodule Main do
  @moduledoc """
  Runnable demo of `PartitionedReg`.
  ...
  def main do
    IO.puts("=== PartitionedReg demo ===")
    run()
    IO.puts("\nDone — demo finished without errors.")
  end

  defp run do
    # PartitionedReg.start_single/1 requires 1 argument(s);
    # call it with real values appropriate for this exercise.
    # PartitionedReg.start_partitioned/2 requires 2 argument(s);
    # call it with real values appropriate for this exercise.
    :ok
  end
end
...
defmodule PartitionedRegTest do
  ...
  describe "run/1" do
    test "returns :ok for a no-op input" do
      assert PartitionedReg.run(:noop) == :ok
    end
  end
end
```
